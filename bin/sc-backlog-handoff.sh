#!/usr/bin/env bash
# Hand already-identified, in-scope backlog items off from the main souschef
...
#
# Scope-matching is souschef's JUDGMENT: you pass the task-id keys you have
...
# active souschef home and appends it, under the same section heading, to the
...
# that is not a souschef home), and is idempotent: a key already present in the
...
# and task lifecycle.
# Usage: sc-backlog-handoff.sh <secondmate-id> <item-key>...
set -eu
...
[ $# -ge 2 ] || { echo "usage: sc-backlog-handoff.sh <secondmate-id> <item-key>..." >&2; exit 1; }
ID=$1
shift
...
validate_secondmate_home() {
  ...
  if [ ! -f "$abs_home/.sc-secondmate-home" ]; then
    echo "error: souschef home $home is not a seeded secondmate home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/.sc-secondmate-home" 2>/dev/null || true)
...
KEYS_FILE=$(mktemp "$MAIN_DIR/.sc-handoff-keys.XXXXXX")
MOVED_FILE=$(mktemp "$MAIN_DIR/.sc-handoff-moved.XXXXXX")
KEPT_FILE=$(mktemp "$MAIN_DIR/.sc-handoff-kept.XXXXXX")
SUB_TMP=$(mktemp "$SUB_DIR/.sc-handoff-sub.XXXXXX")
MAIN_BAK=$(mktemp "$MAIN_DIR/.sc-handoff-main-bak.XXXXXX")
SUB_BAK=$(mktemp "$SUB_DIR/.sc-handoff-sub-bak.XXXXXX")
CHANGES_STARTED=0
COMMITTED=0
cleanup() {
