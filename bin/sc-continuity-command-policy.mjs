...
// by sc-arm-command-policy.mjs. This policy only identifies executed souschef
// fleet scripts and divides them into recovery commands (wake drain, watcher
// arm, and fail-closed teardown) versus every other bin/sc-*.sh command.
// Unparseable or opaque dynamic commands fail open so this gate can never become
// a blanket shell block.

import path from "node:path";
import { fileURLToPath } from "node:url";
import { Lexer, commandPosition, splitProgram } from "./sc-arm-command-policy.mjs";

const RECOVERY_SCRIPTS = new Set(["sc-wake-drain.sh", "sc-watch-arm.sh", "sc-teardown.sh"]);
...
function fleetScript(value, root) {
  ...
  if (!/^sc-[A-Za-z0-9._-]+\.sh$/.test(name)) return "";
...
function fleetScriptRecord(name, position, scriptIndex) {
  ...
  const unsafeTeardown = name === "sc-teardown.sh"
