...
#
# A stray persistent top-level `cd projects/<clone>` in the PRIMARY souschef
# shell silently relocates the shell, so a later souschef-owned command (a
# backlog write, an sc-* lifecycle call) runs inside a project clone instead of
# the home. This seatbelt denies such a command before it runs.
# bin/sc-cd-command-policy.mjs is the sole owner of the block/allow decision; it
# reuses the shell classifier owned by bin/sc-arm-command-policy.mjs. This
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/sc-cd-pretool-check.sh
#   bin/sc-cd-pretool-check.sh --command '<cmd>' [--claude]
#
...
# those. This marker set is COUPLED to the classifier's decoder set in
# bin/sc-arm-command-policy.mjs: adding any new quote/expansion form the
# classifier decodes REQUIRES extending it here in the same change.
PREFILTER=$CMD
PREFILTER=${PREFILTER//\\/}
PREFILTER=${PREFILTER//\"/}
PREFILTER=${PREFILTER//\'/}
...
SC_ROOT=${SC_ROOT_OVERRIDE:-$(CDPATH='' cd -- "$SCRIPT_DIR/.." 2>/dev/null && pwd -P)} || exit 0

# Scope to a plain, non-worktree souschef checkout, where git-dir equals
# git-common-dir. A crewmate/scout task worktree - the shape bin/sc-spawn.sh
# always hands out - is a linked git worktree where the two differ. This guard
# does not inspect .sc-secondmate-home, so it applies in a git-cloned station-chef
# home but remains inert when that home is itself a linked worktree. Any failure
# to confirm the checkout is inert (exit 0), never a block, so a broken
# environment never denies a shell command.
[ -f "$SC_ROOT/AGENTS.md" ] || exit 0
[ -d "$SC_ROOT/bin" ] || exit 0
command -v git >/dev/null 2>&1 || exit 0
GIT_DIR=$(git -C "$SC_ROOT" rev-parse --git-dir 2>/dev/null) || exit 0
GIT_COMMON_DIR=$(git -C "$SC_ROOT" rev-parse --git-common-dir 2>/dev/null) || exit 0
[ "$GIT_DIR" = "$GIT_COMMON_DIR" ] || exit 0

POLICY="$SC_ROOT/bin/sc-cd-command-policy.mjs"
