#!/usr/bin/env bash
# sc-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/sc-watch.sh: runs it as a child, classifies each wake reason, and
# either SELF-HANDLES the routine majority in bash (no souschef turn) or
...
# souschef context; only done/needs-decision/blocked/failed/persistent-wedge/
...
# user message clears it and souschef resumes full per-wake responsiveness.
# When afk is off, the daemon stays quiet — it self-handles routine wakes and
# buffers escalations without injecting, so the base one-shot sc-watch.sh
...
# SC_INJECT_MARK (ASCII unit separator, 0x1f) — a byte a human would never type
# at the start of a message. Souschef's contract: a message that starts with
...
#     injection is recovered on the next sc-wake-drain.sh. The daemon does not
...
#     undelivered past SC_MAX_DEFER_SECS, the daemon retries a normal flush and
...
#
# Usage: sc-supervise-daemon.sh
#          Long-lived background loop. Normally started by the /afk skill, which
#          sets state/.afk first. Env knobs:
#          SC_SUPERVISOR_TARGET     supervisor tmux target (override; otherwise
#                                   auto-discovered from TMUX_PANE, then
#                                   souschef:0 fallback)
#          SC_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   captain-relevant escalation for matching
#                                   kinds.
#          SC_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          SC_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          SC_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          SC_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          SC_BUSY_REGEX            OR-ed busy signatures (mirrors sc-watch.sh)
#          SC_COMPOSER_IDLE_RE      empty-composer regex applied after dim-ghost
#                                   and structural border stripping (default:
#                                   bare prompt glyphs plus busy footers)
#          SC_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          SC_WEDGE_ALARM_CHANNEL   override config/wedge-alarm with a single
#                                   channel directive (off | auto | osascript |
#                                   herdr | command:<cmd>). The backend-
#                                   independent active alert fires when an away-
#                                   mode injection wedges past max-defer so the
#                                   stall is never silent. Default-on (auto ->
#                                   macOS osascript); `off` is the kill switch.
#                                   See wedge_alarm_notify and docs/wedge-alarm.md.
#          SC_WEDGE_ALARM_TIMEOUT_SECS seconds allowed for each notifier before
#                                   its watchdog terminates it (default 10)
#          SC_WEDGE_ALARM_EXEC      notifier seam: when set, every notifier
#                                   channel routes to this command instead of
#                                   the real osascript/herdr/command notifier
#                                   (the special value "discard" fires nothing).
#                                   Sourcing this file defaults it to "discard"
#                                   so no test can post a real notification;
#                                   production execs the daemon and leaves it
#                                   unset (wedge_alarm_emit below).
#          SC_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/sc-tmux-lib.sh):
...
#          SC_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          SC_LOG_MAX_BYTES / SC_LOG_KEEP_LINES / SC_CRASH_*  log + crash guards
#          SC_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing sc-watch.sh is logged and restarted, never killing
#          the daemon; a tight crash-restart spin is detected and backed off.
set -u

SC_DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SC_ROOT="${SC_ROOT_OVERRIDE:-$(cd "$SC_DAEMON_DIR/.." && pwd)}"
...
. "$SC_DAEMON_DIR/sc-tmux-lib.sh"

# --- tunables ---------------------------------------------------------------
SC_SUPERVISOR_TARGET_DEFAULT="souschef:0"
...
# Backend-independent wedge-alarm active-alert channels (see wedge_alarm_notify).
WEDGE_ALARM_TIMEOUT_SECS_DEFAULT=10
WEDGE_ALARM_LAST_EPOCH=0
WEDGE_ALARM_NOTIFIER_PID=
CAPTAIN_RE_DEFAULT='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'
# Busy footers + composer-empty detection now live in bin/sc-tmux-lib.sh
# (SC_TMUX_BUSY_REGEX_DEFAULT / sc_tmux_composer_state); SC_BUSY_REGEX still
...
# injection is prefixed with this byte; souschef treats a leading marker as an
...
# Resolve the effective state dir. SC_STATE_OVERRIDE wins (testing); otherwise
# $SC_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${SC_STATE_OVERRIDE:-$SC_HOME/state}"; }

# --- portable stat (same trap as sc-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by souschef on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes souschef's afk-exit contract as a testable function.
...
# marker (a daemon escalation), 1 otherwise (a real user message). Souschef's
...
# distilled content souschef relays to the captain or feeds back to classifiers.
strip_injection_marker() {  # <message-text>
  local msg=$1
  printf '%s' "${msg#"$SC_INJECT_MARK"}"
}
...
#   1. SC_SUPERVISOR_TARGET env (explicit override) — caller passes it in.
#   2. $TMUX_PANE — tmux sets this in every pane's environment; inherited by
#      the daemon when the /afk skill launches it from souschef's own pane.
#   3. souschef:0 — legacy fallback (may not resolve if the session is named
...
discover_supervisor_target() {
  if [ -n "${SC_SUPERVISOR_TARGET:-}" ]; then
    printf '%s' "$SC_SUPERVISOR_TARGET"
    return 0
  fi
  if [ -n "${TMUX_PANE:-}" ]; then
    printf '%s' "$TMUX_PANE"
  ...
  printf '%s' "$SC_SUPERVISOR_TARGET_DEFAULT"
  return 1
}
...
status_is_captain_relevant() {
  local line=$1
  [ -n "$line" ] || return 1
  printf '%s' "$line" | grep -qiE "${SC_CAPTAIN_RE:-$CAPTAIN_RE_DEFAULT}"
}

# task id from a tmux window name "<session>:sc-<id>" -> "<id>"
window_to_task() {
  local w=$1 t
...
# summary souschef would otherwise have to re-read.
...
classify_check() {  # <full reason>  — check scripts print only when souschef should wake
  printf 'escalate|%s' "$1"
}
...
# Busy + composer-empty detection are the shared primitives in sc-tmux-lib.sh
# (one source of truth with sc-send.sh). These thin wrappers keep the daemon's
...
pane_is_busy() { sc_pane_is_busy "$@"; }        # <window>
pane_input_pending() { sc_pane_input_pending "$@"; }  # <target>
...
# (e.g. a claude-on-herdr primary) gets NO active signal from it - only the
# passive state/.subsuper-inject-wedged marker, which nothing surfaces until the
# next fleet action. That is the exact blind spot behind a documented ~8.5h
# overnight wedge upstream. These helpers add a configurable active alert that
# does not depend on any pane or its backend status-line: an OS-level macOS
# notification, a herdr notification, or a captain-supplied command (push to a
# phone, etc.). Every channel is best-effort - a missing or failing channel logs
# and is skipped, never crashing the daemon loop - and the durable marker plus
# the tmux flash stay exactly as before.
#
# Config: config/wedge-alarm (local, gitignored), one channel directive per
# non-empty, non-comment line. SC_WEDGE_ALARM_CHANNEL overrides the file with a
...
# Print the configured channel directives, one per line. SC_WEDGE_ALARM_CHANNEL
...
wedge_alarm_configured_channels() {
  ...
  if [ -n "${SC_WEDGE_ALARM_CHANNEL:-}" ]; then
    printf '%s\n' "$SC_WEDGE_ALARM_CHANNEL"
    return 0
  fi
  cfg="${SC_CONFIG_OVERRIDE:-$SC_HOME/config}/wedge-alarm"
  if [ -f "$cfg" ]; then
...
# Run a notifier in its own process group under a wall-clock watchdog, so a hung
# channel can never stall the daemon loop. Uses `set -m` + a group kill rather
# than the `timeout` binary (absent on stock macOS). Returns the notifier's exit
# code, 124 on timeout, or 125 if the watchdog could not start.
wedge_alarm_run_bounded() {
  local channel=$1 timeout monitor_was_on=0 pid start elapsed rc
  shift
  timeout=${SC_WEDGE_ALARM_TIMEOUT_SECS:-$WEDGE_ALARM_TIMEOUT_SECS_DEFAULT}
  case "$timeout" in
...
# Terminate the currently-running bounded notifier's process group, if any.
# Called from the run-bounded timeout path and from the daemon shutdown trap.
wedge_alarm_stop_active_notifier() {
...
# SC_WEDGE_ALARM_EXEC, when set, REPLACES the real notifier: the resolved channel
...
# notification - the library-mode guard at the foot of this file defaults it to
# "discard" whenever the daemon is SOURCED rather than executed, which is the
# only way a test reaches these functions. The special value "discard" fires
# nothing; unset means production (the executed daemon), so the real channels
# fire.
wedge_alarm_os_notifier_override() {  # <channel> <summary>
  local channel=$1 summary=$2 rc exec_override=${SC_WEDGE_ALARM_EXEC:-}
...
wedge_alarm_via_osascript() {  # <summary>
  ...
  wedge_alarm_run_bounded osascript osascript -e 'on run argv' \
    -e 'display notification (item 1 of argv) with title "souschef: away-mode escalations WEDGED" sound name "Basso"' \
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "souschef: away-mode escalations WEDGED" \
...
wedge_alarm_via_command() {  # <cmd> <summary>
  ...
  wedge_alarm_run_bounded command sh -c "$cmd" sc-wedge-alarm "$summary" \
...
# Dispatch one channel through the test-forced seam first, then the real
# notifier. SC_WEDGE_ALARM_EXEC set -> the seam handles it (recorder/discard in
# tests); unset -> the real osascript/herdr/command path runs.
wedge_alarm_emit() {  # <channel> <summary> [cmd]
  local channel=$1 summary=$2 cmd=${3:-} rc exec_override=${SC_WEDGE_ALARM_EXEC:-}
  local WEDGE_ALARM_EMIT_ACTIVE=1
  case "$exec_override" in
    '') ;;
    discard) return 0 ;;
    *)
...
# a durable marker souschef/recovery can surface, flashes the tmux supervisor
# client's status line when applicable, and attempts a configurable backend-
# independent active alert (wedge_alarm_notify). Nothing is lost - the buffer and
# the wake-queue both survive - but the stall stops being invisible.
inject_wedge_alarm() {  # <state> <age-seconds>
  local state=$1 age=$2 marker target now notify=1
  marker="$state/.subsuper-inject-wedged"
  # Re-alarm at most once per max-defer window so a long wedge does not spam.
  local max_defer="${SC_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}"
  ...
  {
    printf 'souschef away-mode inject WEDGED: %ss undelivered as of %s\n' "$age" "$(date '+%Y-%m-%dT%H:%M:%S%z')"
    printf 'The supervisor pane could not accept an escalation. Buffered items:\n'
    cat "$state/.subsuper-escalations" 2>/dev/null
  } > "$marker" 2>/dev/null || true
  target="${SC_SUPERVISOR_TARGET:-$SC_SUPERVISOR_TARGET_DEFAULT}"
  # Best-effort status-line flash. tmux's display-message is a client-side OSD
  # with no herdr equivalent; the log line + durable marker above are already the
  # primary signal, so a non-tmux backend just skips this cosmetic extra.
  tmux display-message -t "$target" "souschef: away-mode escalations WEDGED ${age}s - see $marker" 2>/dev/null || true
  ...
  # its status-line is unreadable. Configurable and best-effort; the marker above
  # stays the durable record whether or not any channel fires.
  if [ "$notify" -eq 1 ]; then
...
housekeeping() {  # <state>
  ...
  # (1) batch flush
  if [ "${SC_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${SC_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  max_defer=${SC_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${SC_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
    # Reconstruct the window name from the key (best-effort: session is unknown,
    # so probe the live sc-* windows for one whose task matches).
  ...
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${SC_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
...
# Find a live sc-* window whose task id matches the given marker key.
window_for_task() {  # <task-key>
  local key=$1 w t
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':sc-' || true); do
    t=$(window_to_task "$w")
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; souschef drives the base one-shot
  ...
  # them. Then prepend the sentinel marker — souschef's afk-exit contract
  # keys off its presence at the start of the message.
  msg=$(_collapse_newlines "$msg")
  msg="${SC_INJECT_MARK}${msg}"
  target="${SC_SUPERVISOR_TARGET:-$SC_SUPERVISOR_TARGET_DEFAULT}"
  ...
  retries=${SC_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${SC_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
  verdict=$(sc_tmux_submit_core "$target" "$msg" "$retries" "$sleep_s" "$sleep_s")
...
should_force_self() {  # <reason>
  local reason=$1 skip="${SC_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (SC_INJECT_SKIP): $reason"
    return
  fi
  case "$reason" in
  ...
  if [ "$action" = "escalate" ]; then
    ...
    mark_escalated_seen "$kind" "$arg" "$state"
    [ "${SC_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
  else
...
# --- log --------------------------------------------------------------------
# Uses LOG set by sc_super_main; harmless no-op-ish if unset (tests source fns
...
trim_log() {
  ...
  [ "$sz" -ge "${SC_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/sc-daemon-log.XXXXXX") || return 0
  tail -n "${SC_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/sc-daemon.test.sh).
# ============================================================================
...
sc_super_main() {
  ...
  # Export SC_STATE_OVERRIDE so the lib resolves the same state dir.
  # shellcheck source=bin/sc-wake-lib.sh
  SC_STATE_OVERRIDE="$STATE" . "$SC_DAEMON_DIR/sc-wake-lib.sh"

  local WATCH="$SC_DAEMON_DIR/sc-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${SC_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${SC_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${SC_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${SC_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${SC_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}

  [ -x "$WATCH" ] || { echo "error: watcher not found or not executable: $WATCH" >&2; exit 1; }

  # --- single instance (portable lock, no flock dependency) ------------------
  if ! sc_lock_try_acquire "$LOCK"; then
    if [ -n "${SC_LOCK_HELD_PID:-}" ]; then
      echo "error: another sc-supervise-daemon is already running (pid $SC_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another sc-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"

  # --- auto-discover the supervisor target (the pane running souschef) -----
  # Priority: SC_SUPERVISOR_TARGET override > $TMUX_PANE (inherited from the
  # pane that launched the daemon, normally souschef's own) > souschef:0
  # fallback. Exporting the result into SC_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="SC_SUPERVISOR_TARGET"
  if [ -z "${SC_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    else
      target_source="FALLBACK(souschef:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no SC_SUPERVISOR_TARGET or TMUX_PANE); falling back to '$discovered' — verify this is souschef's pane" >&2
  fi
  SC_SUPERVISOR_TARGET="$discovered"
  local TARGET="$SC_SUPERVISOR_TARGET"
  ...
  if ! tmux display-message -p -t "$TARGET" '#{pane_id}' >/dev/null 2>&1; then
    echo "error: supervisor target '$TARGET' does not resolve to a tmux pane; set SC_SUPERVISOR_TARGET" >&2
    log "startup failed: target '$TARGET' not found"
    sc_lock_release "$LOCK" 2>/dev/null || true
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; afk=$afk_status; inject_skip='${SC_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${SC_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${SC_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"

  # --- shutdown: flush buffered escalations, reap child, release lock -------
  local WATCHER_PID="" CUR_TMP=""
  cleanup() {
    trap - TERM INT
    wedge_alarm_stop_active_notifier 2>/dev/null || true
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/sc-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and souschef itself is the consumer of escalations.
    ...
    if [ -z "${WATCHER_PID:-}" ] || ! kill -0 "${WATCHER_PID:-}" 2>/dev/null; then
      if [ -n "${WATCHER_PID:-}" ]; then
        ...
        if ! is_wake_reason "$reason"; then
          ...
          WATCHER_PID=""
          sleep "${SC_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}"
          continue
        fi
        log "wake: $reason"
        handle_wake "$reason" "$STATE"
    ...
    # The watcher child runs on its own SC_POLL cadence internally; we only need
    ...
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${SC_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  # execs the daemon, see the /afk skill). Make it structurally impossible for a
  # sourced context to fire a real desktop notification from the wedge alarm:
  # default the SC_WEDGE_ALARM_EXEC notifier seam to "discard" unless the embedder
  # already wired one (e.g. a recorder in tests/wake-helpers.sh). It is exported
  # so a real daemon a test later spawns inherits the safe default too. The
  # executed branch above never runs this, so production is untouched.
  : "${SC_WEDGE_ALARM_EXEC:=discard}"
  export SC_WEDGE_ALARM_EXEC
fi
