#!/usr/bin/env bash
# sc-herdr-ci-cleanup.sh - bounded cleanup of CI-owned herdr lab sessions.
#
# Snapshot the session list before a real-herdr suite, then at job end only
# stop/delete sessions that:
#   1. match the guarded sc-lab-* name pattern (what sc-herdr-lab.sh creates),
...
#
# Usage:
#   sc-herdr-ci-cleanup.sh snapshot <path>
#   sc-herdr-ci-cleanup.sh teardown <snapshot-path>
#
# Adapted from the firstmate reference (github.com/kunchenguid/firstmate,
# bin/fm-herdr-ci-cleanup.sh): sc- naming, sc-lab- session prefix.
set -eu

die() {
  printf 'sc-herdr-ci-cleanup.sh: %s\n' "$*" >&2
  exit 1
}
...
cmd=${1:-}
path=${2:-}
[ -n "$cmd" ] && [ -n "$path" ] || die "usage: sc-herdr-ci-cleanup.sh snapshot|teardown <path>"
...
is_lab_name() {
  local name=$1
  [[ "$name" =~ ^sc-lab-[a-zA-Z0-9][a-zA-Z0-9_-]*$ ]]
}
...
case "$cmd" in
  ...
  teardown)
    ...
    candidates=$(printf '%s' "$after_json" | jq -r --argjson before "$before" '
      ...
      | select(.name | test("^sc-lab-[a-zA-Z0-9][a-zA-Z0-9_-]*$"))
      | select((.name as $n | $before | index($n) | not))
      | .name
    ')
    failed=0
    if [ -z "$candidates" ]; then
      log "no job-owned sc-lab-* sessions to clean"
      exit 0
    fi
