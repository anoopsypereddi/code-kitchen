...
usage() {
  echo "usage: sc-home-seed.sh <id> <home|-> <project>..." >&2
  echo "       sc-home-seed.sh validate" >&2
}
...
registry_summary_for_brief() {
  local brief=$1
  if [ -n "${SC_SECONDMATE_CHARTER:-}" ]; then
    printf '%s\n' "$SC_SECONDMATE_CHARTER" | normalize_registry_text
  else
...
registry_scope_for_brief() {
  local brief=$1
  if [ -n "${SC_SECONDMATE_SCOPE:-}" ]; then
    printf '%s\n' "$SC_SECONDMATE_SCOPE" | normalize_registry_text
  else
...
validate_registry() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/sc-souschefs.XXXXXX")
  if [ -f "$REG" ]; then
    while IFS= read -r line; do
      case "$line" in
        "- "*)
...
refuse_active_home_path() {
  ...
  abs_active_home=$(resolved_path "$SC_HOME")
  abs_root=$(resolved_path "$SC_ROOT")
  if [ "$abs_home" = "/" ]; then
...
validate_project_destination() {
  ...
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active souschef home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the souschef repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
ensure_home() {
  ...
  if [ -e "$home" ]; then
    ...
    git clone --quiet "$SC_ROOT" "$home"
  fi
  verify_souschef_home "$home"
}

verify_souschef_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not a souschef home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not a souschef home (missing bin/)" >&2; return 1; }
...
seed_rollback_target() {
  ...
  abs_home=$(resolved_path "$SC_HOME")
  abs_root=$(resolved_path "$SC_ROOT")
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active souschef home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the souschef repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active souschef home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the souschef repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active souschef home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the souschef repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_home() {
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sc-home-seed.XXXXXX")
  ...
  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${SC_SECONDMATE_CHARTER:-}" ] || {
      echo "error: no filled secondmate charter brief at $SEED_PARENT_BRIEF; set SC_SECONDMATE_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    "$SC_ROOT/bin/sc-brief.sh" "$id" --secondmate "$@"
