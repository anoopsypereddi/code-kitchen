...
# worktree, so it cannot discard unlanded work; set SC_FLEET_PRUNE=0 to disable it.
# Usage: sc-fleet-sync.sh [<project-dir>]
set -eu
...
usage() {
  echo "usage: sc-fleet-sync.sh [<project-dir>]" >&2
}
...
prune_gone_branches() {
  ...
  # net. Set SC_FLEET_PRUNE=0 to skip pruning entirely.
  [ "${SC_FLEET_PRUNE:-1}" != "0" ] || return 0
...
sync_project() {
  ...
  mode_line=$("$SC_ROOT/bin/sc-project-mode.sh" "$label" 2>/dev/null || echo "no-mistakes off")
  mode=${mode_line%% *}
