...
# Usage:
#   sc-dispatch-select.sh [--select <strategy>] [--quota-json <file>] [<rule-or-use-json>]
#
# Souschef reads config/crew-dispatch.json, picks the best matching rule with
# judgment, then hands that rule (or its `use` profile/array) to this script to
# turn into ONE concrete {harness, model, effort} it can pass to sc-spawn.sh as
# explicit --harness/--model/--effort flags. The shell never matches the
# natural-language `when` rules; that is souschef's job.
#
...
# SC_DISPATCH_QUOTA_AXI overrides the quota command.
# SC_DISPATCH_STALE_CLEAR_MARGIN overrides the default 20 point stale margin.
set -u

STALE_CLEAR_MARGIN=${SC_DISPATCH_STALE_CLEAR_MARGIN:-20}
SELECT_OVERRIDE=
QUOTA_JSON_FILE=
ARGS=()
...
if [ -n "$QUOTA_JSON_FILE" ]; then
  ...
  quota_cmd=${SC_DISPATCH_QUOTA_AXI:-quota-axi}
