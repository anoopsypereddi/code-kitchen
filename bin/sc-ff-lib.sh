...
# Shared fast-forward machinery for souschef self-sync.
# Usage: . bin/sc-ff-lib.sh   (after SC_ROOT and SC_HOME are set)
#
...
SUB_HOME_MARKER="${SUB_HOME_MARKER:-.sc-secondmate-home}"
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="secondmate $name directory cannot be inside the active souschef home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="secondmate $name directory cannot be inside the souschef repo"
      return 1
    fi
  done
}
...
validate_secondmate_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$SC_HOME") || {
    VALIDATION_ERROR="active souschef home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$SC_ROOT") || {
    VALIDATION_ERROR="souschef repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
  ...
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="secondmate home cannot be the active souschef home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="secondmate home cannot be the souschef repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="secondmate home cannot be inside the active souschef home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="secondmate home cannot be inside the souschef repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="secondmate home cannot be an ancestor of the active souschef home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="secondmate home cannot be an ancestor of the souschef repo"
    return 1
  fi
  ...
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not a souschef home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not a souschef home (missing bin/)"
    return 1
  fi
...
# souschef repo itself (SC_ROOT) is never processed as its own secondmate, and
# each resolved home is processed at most once.
process_secondmate() {
  local id=$1 home=$2 window=${3:-} base_mode=$4 nudge_requires_instr=${5:-no} home_real sc_root_real
  [ -n "$id" ] || return 0
  [ -n "$home" ] || return 0
  sc_root_real=$(resolve_path "$SC_ROOT")
  home_real=$(resolve_path "$home")
  [ "$home_real" != "$sc_root_real" ] || return 0
