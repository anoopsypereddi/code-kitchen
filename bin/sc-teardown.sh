...
validate_removal_target() {
  ...
  [ -e "$target" ] || return 0
  abs_target=$(removal_target_abs_path "$target")
  if abs_home=$(cd "$SC_HOME" 2>/dev/null && pwd -P); then
    :
  else
    abs_home=
  fi
  abs_root=$(cd "$SC_ROOT" && pwd -P)
  case "$abs_target" in
    ''|/) echo "REFUSED: unsafe $label removal target $target" >&2; return 1 ;;
  esac
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active souschef home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the souschef repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active souschef home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the souschef repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active souschef home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the souschef repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_souschef_operational_dirs_for_removal() {
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$SC_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active souschef home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$SC_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the souschef repo" >&2
    return 1
  fi
...
validate_souschef_home_for_removal() {
  ...
  validate_souschef_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
...
validate_souschef_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      validate_souschef_home_for_removal "$child_home" "child souschef home" "$child_id" >/dev/null || return 1
      validate_souschef_home_children_removal "$child_home" || return 1
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_souschef_home_children "$child_home"
        remove_souschef_home "$child_home" "child souschef home" "$child_id"
      fi
    elif [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
      validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
...
if [ "$KIND" = secondmate ]; then
  ...
  validate_souschef_home_for_removal "$HOME_PATH" "secondmate home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_souschef_home_children_removal "$HOME_PATH" || exit 1
  fi
fi
...
if [ "$KIND" = secondmate ] && [ "$FORCE" = "--force" ]; then
  cleanup_souschef_home_children "$HOME_PATH"
fi
...
if [ -d "$WT" ] && [ "$FORCE" != "--force" ]; then
  if [ "$KIND" = secondmate ]; then
    ...
    if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
      ...
      # into the local default branch (souschef does that merge on the captain's
      ...
      if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
        ...
        echo "Merge the branch into local $DEFAULT first (bin/sc-merge-local.sh after the captain approves), or push to a fork/remote, or get the captain's explicit OK to discard, then --force." >&2
        exit 1
      fi
    elif [ -n "$dirty" ]; then
