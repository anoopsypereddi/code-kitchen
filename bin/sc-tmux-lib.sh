#!/usr/bin/env bash
# sc-tmux-lib.sh — shared tmux pane primitives for souschef.
#
...
# daemon (bin/sc-supervise-daemon.sh) and bin/sc-send.sh so the composer/submit
...
# idle pane as holding pending input and the daemon deferred injection / souschef
...
# surfaced (sc-peek and every human/LLM-facing path stay plain), and only the
...
#
# Per-harness override: SC_COMPOSER_IDLE_RE matches an empty composer after
# dim-ghost and structural border stripping. SC_BUSY_REGEX overrides the busy
# footer set (mirrors sc-watch.sh / the daemon).
#
# All functions are `set -u` and `set -e` safe (guarded tmux calls, explicit
# returns) so they can be sourced into either context.

# Busy footers per harness (mirror sc-watch.sh). claude/codex: "esc to
# interrupt"; opencode: "esc interrupt"; pi: "Working...".
SC_TMUX_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working\.\.\.'

# sc_tmux_strip_ghost: remove dim/faint (ANSI SGR 2) styled runs from one captured
...
# sc_tmux_composer_state: classify the cursor/composer line of <target> as
...
# the single composer row (-S/-E), then run through sc_tmux_strip_ghost so dim/faint
...
sc_tmux_composer_state() {  # <target> -> empty|pending|unknown
  ...
  if [ -n "${SC_COMPOSER_IDLE_RE:-}" ] \
     && printf '%s' "$stripped" | grep -qiE "$SC_COMPOSER_IDLE_RE"; then
  ...
  if printf '%s' "$stripped" | grep -qiE "${SC_BUSY_REGEX:-$SC_TMUX_BUSY_REGEX_DEFAULT}"; then
    printf 'empty'; return 0
  fi
  printf 'pending'; return 0
}

# sc_pane_input_pending: 0 (pending) if the cursor line holds real unsubmitted
...
sc_pane_input_pending() {  # <target>
  [ "$(sc_tmux_composer_state "$1")" = pending ]
}

# sc_pane_is_busy: 0 if the pane's last few non-blank lines show a busy footer
# (an agent mid-turn). Scans a 40-line tail like sc-watch.sh.
sc_pane_is_busy() {  # <target>
  local win=$1 tail40
  tail40=$(tmux capture-pane -p -t "$win" -S -40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | grep -qiE "${SC_BUSY_REGEX:-$SC_TMUX_BUSY_REGEX_DEFAULT}"
}

# sc_tmux_submit_core: type <text> into <target> ONCE, then submit with Enter,
...
#   - sc-send fails only on "pending" (lenient: a positively-confirmed swallow),
#     so an unreadable pane never turns a normal steer into a false error.
sc_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep>
  local target=$1 retries=$2 sleep_s=$3 i=0 state
  while :; do
    tmux send-keys -t "$target" Enter 2>/dev/null || true
    sleep "$sleep_s"
    state=$(sc_tmux_composer_state "$target")
...
sc_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5
  tmux send-keys -t "$target" -l "$text" 2>/dev/null || { printf 'send-failed'; return 0; }
  sleep "$settle"
  sc_tmux_submit_enter_core "$target" "$retries" "$sleep_s"
}
