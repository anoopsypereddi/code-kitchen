...
//
// This file is the sole owner of souschef's shell command classification.
// The tokenizer and command-position analysis (Lexer, splitProgram,
// commandPosition) are exported so the sibling policies
// (bin/sc-cd-command-policy.mjs, bin/sc-continuity-command-policy.mjs) reuse the
// same proven parser instead of duplicating shell lexing. The watcher-arm
// decision procedure below stays private to this file. The CLI entry point at
// the bottom runs only when this module is invoked directly, never on import.
//
// The protected watcher command is bin/sc-watch-arm.sh (the verified re-arm);
// bin/sc-watch.sh must never be run directly. The arm must be a standalone,
// harness-tracked background call - never a shell `&`/nohup/disown detach, a
// pipeline stage, a redirection, or bundled behind other commands - so the
// dying child cannot report a false "already running" that hides a lapsed pass.
...
const REASONS = {
  "watcher-background": "a protected watcher command cannot run in an asynchronous shell list or through nohup/disown; arm it as a harness-tracked background task, never a shell &",
  ...
  "broad-watcher-kill": "a broad process kill targeting the souschef watcher is forbidden; use bin/sc-watch-arm.sh --restart to restart only this home's pass",
  "unclassifiable-protected-command": "unsupported or malformed shell syntax contains a protected watcher command",
  "watcher-direct": "bin/sc-watch.sh must not be run directly; arm the watcher with bin/sc-watch-arm.sh instead",
};
...
const PROTECTED_SCRIPTS = [
  { relative: "bin/sc-watch-arm.sh", kind: "arm" },
  { relative: "bin/sc-watch.sh", kind: "watch" },
];
...
function hasUnclassifiableProtectedExpansion(word, root) {
  ...
  return /(?:^|\/)sc-watch/.test(word.value);
}
...
function contextWithAssignments(context, words) {
  ...
  for (const word of words) {
    ...
    if (/sc-watch/.test(value) || wordReferencesAny(word, watcherPatterns)) watcherPatterns.add(name);
...
function isWatcherPgrep(position, context) {
  ...
  return position.words.slice(position.index + 1).some((word) => /(?:^|\/)sc-watch(?:\.sh)?\b/.test(word.value) || wordReferencesAny(word, context.watcherPatterns));
}
...
function analyzeProgram(command, context, depth = 0) {
  ...
  for (const tokens of program.nodes) {
    ...
    if (commandName === "pkill" && args.some((word) => /sc-watch/.test(word.value) || wordReferencesAny(word, nodeContext.watcherPatterns))) broadKill = true;
...
// Approved setup nodes that may precede the final protected arm command: a plain
// `cd <dir>` and a plain `export VAR=val`. Souschef has no X-mode, so the
// x-mode.env source/test-source setup allowances from firstmate are dropped.
...
function decision(command, root) {
  const context = { root: path.normalize(root), protectedVariables: new Set(), watcherPatterns: new Set(), watcherPids: new Set() };
...
// Run the CLI only when invoked directly (node sc-arm-command-policy.mjs ...),
// never when imported by a sibling policy such as bin/sc-cd-command-policy.mjs.
