...
# by sc-wake-drain.sh after it empties queued wakes.
# First, always warn if the souschef primary checkout (SC_ROOT) is on a named
# non-default branch, because that means souschef-on-itself work landed in the
...
# souschef PRIMARY checkout (SC_ROOT) must stay on its default branch. If a
...
tangle_branch=$(sc_primary_tangle_branch "$SC_ROOT" || true)
if [ -n "$tangle_branch" ]; then
  ...
  trule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    printf '●      git -C %s checkout %s\n' "$SC_ROOT" "$tangle_default"
    printf "●  then re-validate '%s' in a proper isolated worktree.\n" "$tangle_branch"
    printf '●%s\n' "$trule"
  } >&2
fi

# Portable mtime; see sc-watch.sh for why the `stat -f || stat -c` fallback breaks on Linux.
...
[ -s "$SC_WAKE_QUEUE" ] && queue_pending=true
...
if "$queue_pending"; then
  echo "WARNING: queued wakes pending - drain them with bin/sc-wake-drain.sh before anything else." >&2
fi
exit 0
