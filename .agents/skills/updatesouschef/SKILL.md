---
name: updatesouschef
description: Self-update a running Souschef and its station chefs to the latest from origin. Use when the Chef invokes /updatesouschef (e.g. "/updatesouschef", "update Souschef", "pull the latest Souschef"). Fast-forwards this Souschef repo's default branch and every station chef home from origin (fast-forward only, never forced, never disruptive), then re-reads AGENTS.md and nudges each updated station chef to do the same, so the whole tree runs the latest bin/ and instructions.
user-invocable: true
---
...
3. **Nudge each updated live station chef.**
   ...
   ```sh
   bin/sc-send.sh <window-target> 'Souschef was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.'
   ```
...
4. **Report to the Chef in plain outcomes.**
   Summarize what landed without Souschef's internal vocabulary: which parts of the brigade are now on the latest, and which were left as-is and why.
   For example: "Chef, Souschef and both domain expediters are now on the latest."
...
- **Only the Souschef repo and its worktrees** are touched, never `projects/`.
