---
name: ahoy
description: Recap visible session events since the prior real Chef message plus visibly unanswered Chef decisions when the Chef explicitly invokes /ahoy, with a /bearings fallback when /ahoy is the session's first real Chef message. Session-history-only - no tools, no state gathering, no cost.
user-invocable: true
---
...
1. Inspect only conversation or session history already visible to the current souschef.
2. Find the most recent real Chef-authored message before the current `/ahoy` invocation.
   ...
   In a secondmate home, also exclude messages carrying the leading from-souschef marker that `bin/sc-send.sh` prepends to routed requests.
...
The current `/ahoy` message is outside the recap interval; a previous `/ahoy` is a real Chef message and may be the next interval boundary.
