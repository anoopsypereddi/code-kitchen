...
Keep the always-inline routing rules in `AGENTS.md` authoritative: route by natural-language `scope:`, local-only projects stay with the main Sous, and station chefs are idle by default.
...
Keep the charter focused on the persistent responsibility, available project clones, escalation back to the main Sous status file, and the requests-from-main-Sous contract.
...
`-` durably leases a fresh Sous worktree via `treehouse get --lease` under the station chef id.
...
`bin/fm-home-seed.sh` copies the charter into the station chef home as `data/charter.md`.
`bin/fm-spawn.sh --secondmate` launches it through the same launch-template path.
Before launch, `fm-spawn.sh --secondmate` locally fast-forwards the home to the primary Sous checkout's current default-branch commit when it is safe; dirty, diverged, or in-flight homes launch unchanged with a warning.
...
`local-only` projects stay with the main Sous.
...
It refuses any destination that is not a genuine seeded Sous home with safe operational directories and a matching `.fm-secondmate-home` marker, so a move can never land in a project.
...
Re-fire uses the same guarded pre-launch sync, so recovered station chefs converge to the primary Sous version without fetching from origin whenever their home can be cleanly fast-forwarded.

Do not reconstruct a station chef's whole tree from the main home.
The main Sous reconciles only direct reports.
Each station chef is a Sous in its own home, so it runs recovery on startup and reconciles its own cooks.
