...
Keep the always-inline routing rules in `AGENTS.md` authoritative: route by natural-language `scope:`, local-only projects stay with the main Souschef, and station chefs are idle by default.
...
Keep the charter focused on the persistent responsibility, available project clones, escalation back to the main Souschef status file, and the requests-from-main-Souschef contract.
...
Before launch, `sc-spawn.sh --secondmate` locally fast-forwards the home to the primary Souschef checkout's current default-branch commit when it is safe; dirty, diverged, or in-flight homes launch unchanged with a warning.
...
It refuses any destination that is not a genuine seeded Souschef home with safe operational directories and a matching `.sc-secondmate-home` marker, so a move can never land in a project.
...
Re-fire uses the same guarded pre-launch sync, so recovered station chefs converge to the primary Souschef version without fetching from origin whenever their home can be cleanly fast-forwarded.

Do not reconstruct a station chef's whole tree from the main home.
The main Souschef reconciles only direct reports.
Each station chef is a Souschef in its own home, so it runs recovery on startup and reconciles its own cooks.
