#!/usr/bin/env bash
# tests/sc-daemon.test.sh - supervise-daemon classifiers, the captain-relevant
...
# sc-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in sc-afk-inject-e2e and sc-wake-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"

DAEMON="$ROOT/bin/sc-supervise-daemon.sh"
...
if [ -z "${SC_TEST_DAEMON_SOURCED:-}" ]; then
  export SC_TEST_DAEMON_SOURCED=1
  # shellcheck source=bin/sc-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(sc_test_tmproot sc-daemon-tests)


test_daemon_state_root_uses_fm_home() {
  local dir home override out
  dir=$(make_supercase daemon-sc-home)
  home="$dir/souschef-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(SC_HOME="$home" SC_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored SC_HOME: $out"

  out=$(SC_HOME="$home" SC_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored SC_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by SC_HOME"
}
...
test_classify_routine_signal_self() {
  ...
  out=$(SC_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
    ...
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(SC_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:sc-qux-w4" "$state"
  out=$(SC_STATE_OVERRIDE="$state" classify_stale "sess:sc-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:sc-qux-w4")" | tr ':/.' '___')
...
test_stale_terminal_escalates() {
  ...
  out=$(SC_STATE_OVERRIDE="$state" classify_stale "sess:sc-fin-t5" "$state")
...
test_housekeeping_persistent_stale_escalates() {
  ...
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_WINDOW="$win" SC_FAKE_TMUX_CAPTURE="$pane" \
    SC_STATE_OVERRIDE="$state" SC_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_escalate_batches_into_one_digest() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_PANE_ALIVE=1 SC_FAKE_TMUX_SENT="$sent" \
    SC_FAKE_TMUX_CAPTURE="$capture" SC_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_PANE_ALIVE=1 SC_FAKE_TMUX_SENT="$sent" \
    SC_FAKE_TMUX_CAPTURE="$capture" SC_ESCALATE_BATCH_SECS=90 SC_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_heartbeat_scan_dedup() {
  ...
  SC_STATE_OVERRIDE="$state" housekeeping "$state"
...
test_handle_wake_routes_self_and_escalate() {
  ...
  SC_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  SC_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  SC_STATE_OVERRIDE="$state" SC_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:sc-x" || fail "stale: not recognized as wake"
...
test_terminal_stale_escalate_leaves_no_marker() {
  ...
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  SC_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  SC_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" SC_FAKE_TMUX_PANE_ALIVE=1 SC_FAKE_TMUX_SENT="$sent" \
    SC_FAKE_TMUX_CAPTURE="$capture" SC_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_busy_guard_defers_when_supervisor_busy() {
  ...
  # pane shows a busy signature (souschef mid-turn)
...
test_marker_detection() {
  ...
  message_is_injection "${SC_INJECT_MARK}Supervisor escalate: done" \
  ...
  state="$dir/state"
  afk_enter "$state"
  should_exit_afk "$state" "${SC_INJECT_MARK}escalate" \
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${SC_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  local stripped
  stripped=$(strip_injection_marker "${SC_INJECT_MARK}Supervisor escalate: done")
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$SC_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_CAPTURE="$capture" SC_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  capture="$dir/pane.txt"
  printf '│ custom idle> │\n' > "$capture"
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_CAPTURE="$capture" SC_FAKE_TMUX_CURSOR_Y=0 \
    SC_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
    && fail "SC_COMPOSER_IDLE_RE was not applied after border stripping"
  pass "pane_input_pending honors SC_COMPOSER_IDLE_RE after border stripping"
}
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(SC_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(SC_STATE_OVERRIDE="$state" classify_stale "sess:sc-dup-s10" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in \
    ...
    "│                                              │"; do
    printf '%s\n' "$line" > "$capture"
    if PATH="$fakebin:$PATH" SC_FAKE_TMUX_CAPTURE="$capture" SC_FAKE_TMUX_CURSOR_Y=0 \
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" SC_FAKE_COMPOSER="$dir/composer" SC_FAKE_SENT="$sent" \
    sc_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, sc-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" SC_FAKE_COMPOSER="$dir/composer" SC_FAKE_SENT="$sent" \
    SC_FAKE_SWALLOW="$dir/.swallow" SC_FAKE_PERSIST_SWALLOW=1 \
    sc_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SC_FAKE_COMPOSER="$dir/composer" SC_FAKE_SENT="$sent" \
    SC_FAKE_SWALLOW="$dir/.swallow" SC_FAKE_PERSIST_SWALLOW=1 SC_INJECT_CONFIRM_SLEEP=0.05 \
    SC_ESCALATE_BATCH_SECS=99999 SC_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SC_FAKE_COMPOSER="$dir/composer" SC_FAKE_SENT="$sent" \
    SC_ESCALATE_BATCH_SECS=99999 SC_MAX_DEFER_SECS=60 SC_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SC_FAKE_COMPOSER="$dir/composer" SC_FAKE_SENT="$sent" \
    SC_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_PANE_ALIVE=1 SC_FAKE_TMUX_SENT="$sent" \
    SC_FAKE_TMUX_CAPTURE="$capture" SC_FAKE_TMUX_CURSOR_Y=0 \
    SC_ESCALATE_BATCH_SECS=99999 SC_MAX_DEFER_SECS=300 housekeeping "$state"
...
test_fm_send_exits_nonzero_on_confirmed_swallow() {
  # sc-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so souschef learns the instruction did not land
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$dir/state" SC_FAKE_COMPOSER="$dir/composer" \
    SC_SEND_SLEEP=0.05 "$ROOT/bin/sc-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "sc-send exited non-zero on a clean submit: $(cat "$err")"
  # Persistent swallow -> exit non-zero with a clear message.
  printf '│ > │\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$dir/state" SC_FAKE_COMPOSER="$dir/composer" \
    SC_FAKE_SWALLOW="$dir/.swallow" SC_FAKE_PERSIST_SWALLOW=1 SC_SEND_SLEEP=0.05 \
    "$ROOT/bin/sc-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    fail "sc-send exited zero despite a swallowed Enter (silent unsubmitted instruction)"
  fi
  grep -F 'not submitted' "$err" >/dev/null || fail "sc-send did not explain the swallowed submit: $(cat "$err")"
  pass "sc-send exits non-zero on a confirmed swallow, zero on a clean submit"
}
...
test_fm_send_exits_nonzero_on_initial_send_failure() {
  ...
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$dir/state" SC_FAKE_COMPOSER="$dir/composer" \
    SC_FAKE_SEND_FAIL=1 SC_SEND_SLEEP=0.05 \
    "$ROOT/bin/sc-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "sc-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "sc-send did not explain initial send failure: $(cat "$err")"
