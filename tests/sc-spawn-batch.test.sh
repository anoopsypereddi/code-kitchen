#!/usr/bin/env bash
# Behavior tests for sc-spawn.sh batch dispatch (`id=repo` pairs).
#
# These exercise argument routing only: each spawn attempt fails fast at the
# missing-brief check, which is reached before any tmux/worktree side effect, so
# the tests create no windows or worktrees. SC_SPAWN_NO_GUARD=1 keeps them off the
...
TMP_ROOT=$(sc_test_tmproot sc-spawn-batch)

# Clear ambient souschef overrides so the behavior test owns its environment.
run_spawn() {
  SC_ROOT_OVERRIDE='' \
    SC_HOME='' \
    SC_STATE_OVERRIDE='' \
    SC_DATA_OVERRIDE='' \
    SC_PROJECTS_OVERRIDE='' \
    SC_CONFIG_OVERRIDE='' \
    SC_SPAWN_NO_GUARD=1 \
    "$SPAWN" "$@" 2>&1
}
...
# A projects/ path is resolved through the souschef home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(SC_ROOT_OVERRIDE='' SC_STATE_OVERRIDE='' SC_DATA_OVERRIDE='' SC_CONFIG_OVERRIDE='' \
        SC_HOME="$home" SC_PROJECTS_OVERRIDE="$projects" SC_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    else
      mkdir -p "$home/projects/alpha"
      out=$(SC_ROOT_OVERRIDE='' SC_STATE_OVERRIDE='' SC_DATA_OVERRIDE='' SC_PROJECTS_OVERRIDE='' SC_CONFIG_OVERRIDE='' \
        SC_HOME="$home" SC_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    fi
    status=$?
...
SC_HOME scopes projects/|no|nope-home-z7
SC_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the souschef home for single-task spawn"
}
