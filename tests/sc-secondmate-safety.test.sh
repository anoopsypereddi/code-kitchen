...
TMP_ROOT=$(sc_test_tmproot sc-secondmate-safety)
...
test_fm_home_parameterization() {
  ...
  out=$(SC_HOME="$home_one" "$ROOT/bin/sc-project-mode.sh" app)
  [ "$out" = "local-only on" ] || fail "sc-project-mode did not read projects.md from SC_HOME"
  out=$(SC_HOME="$home_two" "$ROOT/bin/sc-project-mode.sh" app 2>/dev/null)
  [ "$out" = "no-mistakes off" ] || fail "sc-project-mode did not isolate missing registry by home"

  SC_HOME="$home_one" "$ROOT/bin/sc-brief.sh" task-a app >/dev/null || fail "brief scaffold failed under SC_HOME"
  brief="$home_one/data/task-a/brief.md"
  [ -f "$brief" ] || fail "brief was not written under SC_HOME/data"
  grep -F ">> '$home_one/state/task-a.status'" "$brief" >/dev/null || fail "brief did not shell-quote SC_HOME state path"

  SC_HOME="$home_one" "$ROOT/bin/sc-brief.sh" task-b app --scout >/dev/null || fail "scout brief scaffold failed under SC_HOME"
  brief="$home_one/data/task-b/brief.md"
  grep -F ">> '$home_one/state/task-b.status'" "$brief" >/dev/null || fail "scout brief did not shell-quote SC_HOME state path"

  SC_HOME="$home_one" SC_SECONDMATE_CHARTER='ops domain' "$ROOT/bin/sc-brief.sh" task-c --secondmate app >/dev/null \
    || fail "secondmate brief scaffold failed under SC_HOME"
  brief="$home_one/data/task-c/brief.md"
  grep -F ">> '$home_one/state/task-c.status'" "$brief" >/dev/null || fail "secondmate brief did not shell-quote SC_HOME state path"

  printf 'project=x\n' > "$home_one/state/task-a.meta"
  SC_HOME="$home_one" SC_GUARD_GRACE=999999 "$ROOT/bin/sc-pr-check.sh" task-a https://github.com/example/repo/pull/1 >/dev/null 2>/dev/null \
    || fail "sc-pr-check failed under SC_HOME"
  [ -f "$home_one/state/task-a.check.sh" ] || fail "pr check was not written under SC_HOME/state"
  [ ! -e "$home_two/state/task-a.check.sh" ] || fail "pr check leaked into another home"
  pass "SC_HOME parameterizes data and state paths"
}
...
test_seed_allows_overlapping_clones_and_drops_owner() {
  ...
  # init, modes preserved - is asserted by sc-secondmate-lifecycle-e2e.
  local home design other
  home="$TMP_ROOT/overlap-main"
  design="$TMP_ROOT/overlap-design"
  other="$TMP_ROOT/overlap-other"
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_init_commit "$home/projects/beta"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/seed-overlap-alpha.git"
  sc_git_add_origin "$home/projects/beta" "$TMP_ROOT/remotes/seed-overlap-beta.git"
  cat > "$home/data/projects.md" <<EOF
- alpha [direct-PR] - alpha project (added 2026-06-22)
- beta [direct-PR] - beta project (added 2026-06-22)
EOF

  SC_HOME="$home" SC_SECONDMATE_CHARTER='feature design for alpha beta' \
    SC_SECONDMATE_SCOPE='feature design for alpha beta' \
    "$ROOT/bin/sc-home-seed.sh" design "$design" alpha beta >/dev/null \
  ...
  SC_HOME="$home" SC_SECONDMATE_CHARTER='issue triage for beta' \
    SC_SECONDMATE_SCOPE='issue triage for beta' \
    "$ROOT/bin/sc-home-seed.sh" other "$other" beta >/dev/null 2>&1 \
  ...
  SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" validate >/dev/null || fail "registry validation rejected overlapping clones"

  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" owner alpha >/dev/null 2>&1; then
...
EOF
  ...
  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" validate >/dev/null 2>"$err"; then
...
test_home_seed_uses_treehouse_acquired_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-alpha.git"
...
test_home_seed_returns_treehouse_acquired_home_on_assignment_failure() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-fail-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  git clone --quiet "$ROOT" "$acquired"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf 'other\n' > "$acquired/.sc-secondmate-home"
...
test_home_seed_warns_when_acquired_home_return_fails() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-return-fail-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  git clone --quiet "$ROOT" "$acquired"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf 'other\n' > "$acquired/.sc-secondmate-home"
...
test_home_seed_does_not_return_unsafe_acquired_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-active-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
...
test_home_seed_rolls_back_failed_clone() {
  ...
  sc_git_init_commit "$home/projects/alpha"
  sc_git_init_commit "$home/projects/beta"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/rollback-alpha.git"
...
EOF
  ...
  if SC_HOME="$home" SC_SECONDMATE_CHARTER='rollback scope' SC_SECONDMATE_SCOPE='rollback scope' \
    "$ROOT/bin/sc-home-seed.sh" rollback "$subhome" alpha beta >/dev/null 2>"$err"; then
  ...
  [ ! -e "$subhome/.sc-secondmate-home" ] || fail "failed seed left a subhome marker"
...
test_home_seed_refuses_missing_filled_charter() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/missing-charter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_placeholder_charter() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/placeholder-charter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  SC_HOME="$home" "$ROOT/bin/sc-brief.sh" design --secondmate alpha >/dev/null \
...
test_home_seed_refuses_empty_charter_fields() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/empty-charter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  if SC_HOME="$home" SC_SECONDMATE_CHARTER='   ' "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
  ...
  SC_SECONDMATE_SCOPE='   ' scaffold_secondmate_charter "$home" design 'filled charter' alpha \
...
test_home_seed_refuses_registry_delimiter_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/delimiter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  if SC_HOME="$home" SC_SECONDMATE_CHARTER='delimiter charter' "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
  ...
  [ ! -e "$subhome/.sc-secondmate-home" ] || fail "delimiter home seed wrote a marker"
...
test_home_seed_refuses_active_home_and_root() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/active-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondmate_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for active-home seed test"

  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$home" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to reuse active SC_HOME"
  fi
  grep -F 'secondmate home cannot be the active souschef home' "$err" >/dev/null \
    || fail "seed did not explain active SC_HOME rejection"

  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$active_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home inside active SC_HOME"
  fi
  grep -F 'secondmate home cannot be inside the active souschef home' "$err" >/dev/null \
    || fail "seed did not explain active SC_HOME descendant rejection"
  [ ! -e "$home/nested" ] || fail "seed created a directory inside active SC_HOME before descendant rejection"

  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$active_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to contain active SC_HOME"
  fi
  grep -F 'secondmate home cannot be an ancestor of the active souschef home' "$err" >/dev/null \
    || fail "seed did not explain active SC_HOME ancestor rejection"
  [ ! -f "$active_ancestor/.sc-secondmate-home" ] || fail "seed marked an ancestor of active SC_HOME"

  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$ROOT" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to reuse SC_ROOT"
  fi
  grep -F 'secondmate home cannot be the souschef repo' "$err" >/dev/null \
    || fail "seed did not explain SC_ROOT rejection"

  git clone --quiet "$ROOT" "$root_clone"
  if SC_HOME="$home" SC_ROOT_OVERRIDE="$root_clone" "$ROOT/bin/sc-home-seed.sh" design "$root_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home inside SC_ROOT"
  fi
  grep -F 'secondmate home cannot be inside the souschef repo' "$err" >/dev/null \
    || fail "seed did not explain SC_ROOT descendant rejection"
  [ ! -e "$root_clone/tmp" ] || fail "seed created a directory inside SC_ROOT before descendant rejection"

  git clone --quiet "$ROOT" "$root_ancestor"
  git clone --quiet "$ROOT" "$root_inside"
  if SC_HOME="$home" SC_ROOT_OVERRIDE="$root_inside" "$ROOT/bin/sc-home-seed.sh" design "$root_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to contain SC_ROOT"
  fi
  grep -F 'secondmate home cannot be an ancestor of the souschef repo' "$err" >/dev/null \
    || fail "seed did not explain SC_ROOT ancestor rejection"
  [ ! -f "$root_ancestor/.sc-secondmate-home" ] || fail "seed marked an ancestor of SC_ROOT"
...
test_home_seed_refuses_home_marked_for_another_id() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/marked-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  printf 'other\n' > "$subhome/.sc-secondmate-home"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  ...
  [ "$(cat "$subhome/.sc-secondmate-home")" = "other" ] || fail "seed overwrote another secondmate marker"
...
test_home_seed_refuses_home_registered_to_another_id() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/registered-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  subhome_abs=$(cd "$subhome" && pwd -P)
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  ...
  [ ! -e "$subhome/.sc-secondmate-home" ] || fail "seed wrote a marker before rejecting a registered home"
...
test_home_seed_refuses_reassigning_existing_id_to_different_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/reassign-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  SC_HOME="$home" SC_SECONDMATE_CHARTER='design domain' SC_SECONDMATE_SCOPE='design domain' \
    "$ROOT/bin/sc-home-seed.sh" design "$first" alpha >/dev/null \
    || fail "initial seed failed for reassigning-id test"
  first_abs=$(cd "$first" && pwd -P)

  if SC_HOME="$home" SC_SECONDMATE_CHARTER='design domain' SC_SECONDMATE_SCOPE='design domain' \
    "$ROOT/bin/sc-home-seed.sh" design "$second" alpha >/dev/null 2>"$err"; then
  ...
  [ "$(cat "$first/.sc-secondmate-home")" = design ] || fail "failed id reassignment changed the original marker"
...
test_home_seed_refuses_home_overlapping_registered_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/overlap-alpha.git"
...
EOF
  ...
  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$nested" alpha >/dev/null 2>"$err"; then
  ...
  if SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$parent" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home containing a registered secondmate home"
  fi
  grep -F 'overlaps registered secondmate home' "$err" >/dev/null \
    || fail "seed did not explain registered descendant overlap"
  [ ! -f "$parent/.sc-secondmate-home" ] || fail "seed marked a home containing a registered home"
...
test_home_seed_refuses_existing_remote_backed_project_with_wrong_origin() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/wrong-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  subhome_abs=$(cd "$subhome" && pwd -P)
...
test_home_seed_resolves_relative_source_origins() {
  ...
  out=$(SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha)
  subhome_abs=$(cd "$subhome" && pwd -P)
  ...
  SC_HOME="$home" "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha >/dev/null \
...
test_home_seed_skips_initialized_existing_no_mistakes_projects() {
  ...
  sc_git_init_commit "$home/projects/alpha"
  sc_git_init_commit "$home/projects/beta"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/existing-alpha.git"
  sc_git_add_origin "$home/projects/beta" "$TMP_ROOT/remotes/existing-beta.git"
  git clone --quiet "$ROOT" "$subhome"
  mkdir -p "$subhome/projects"
  ...
  if PATH="$fakebin:$PATH" SC_FAKE_NO_MISTAKES_LOG="$log" SC_FAKE_NO_MISTAKES_FAIL_PROJECT=beta \
    SC_HOME="$home" SC_SECONDMATE_CHARTER='existing init rollback scope' SC_SECONDMATE_SCOPE='existing init rollback scope' \
    "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha beta >/dev/null 2>"$err"; then
...
test_home_seed_refuses_uninitialized_existing_no_mistakes_project() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/uninitialized-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  mkdir -p "$subhome/projects"
  origin=$(git -C "$home/projects/alpha" remote get-url origin)
  git clone --quiet "$origin" "$subhome/projects/alpha"
  ...
  if PATH="$fakebin:$PATH" SC_FAKE_NO_MISTAKES_LOG="$log" \
    SC_HOME="$home" SC_SECONDMATE_CHARTER='existing uninitialized scope' \
    "$ROOT/bin/sc-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_project_destinations_outside_subhome() {
  ...
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/symlink-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  ...
  [ ! -f "$subhome/.sc-secondmate-home" ] || fail "seed marked subhome after unsafe project destination rejection"
...
test_home_seed_refuses_operational_dirs_outside_subhome() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/symlink-opdir-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  ...
  for opdir in data state config; do
    ...
    [ ! -f "$subhome/.sc-secondmate-home" ] || fail "seed marked subhome after unsafe $opdir directory rejection"
  done
...
test_home_seed_refuses_symlinked_leaf_files() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  sc_git_init_commit "$home/projects/alpha"
  sc_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/symlink-leaf-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondmate_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink leaf seed test"

  for leaf in data/projects.md data/charter.md .sc-secondmate-home; do
    ...
    expected=outside
    if [ "$leaf" = ".sc-secondmate-home" ]; then
      expected=design
    fi
    printf '%s\n' "$expected" > "$sink"
    ...
    [ ! -f "$subhome/.sc-secondmate-home" ] || [ "$leaf" = ".sc-secondmate-home" ] || fail "seed marked subhome after symlinked leaf refusal"
  done
...
test_secondmate_spawn_requires_seeded_matching_home() {
  ...
  cat > "$fakeroot/bin/sc-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fakeroot/bin/sc-guard.sh"
  mkdir -p "$ancestor_active_home/data" "$ancestor_active_home/state" "$active_ancestor/data" "$root_ancestor/data" "$root_inside/bin"
  cat > "$root_inside/bin/sc-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root_inside/bin/sc-guard.sh"
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$subhome" codex --secondmate >/dev/null 2>"$err"; then
  ...
  printf 'other\n' > "$wronghome/.sc-secondmate-home"
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$wronghome" codex --secondmate >/dev/null 2>"$err"; then
  ...
  printf 'domain\n' > "$marker_only/.sc-secondmate-home"
  printf 'charter\n' > "$marker_only/data/charter.md"
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$marker_only" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a marked home missing AGENTS.md"
  fi
  grep -F 'not a souschef home (missing AGENTS.md)' "$err" >/dev/null || fail "spawn did not explain missing AGENTS.md"
  ...
  grep -F 'not a souschef home (missing bin/)' "$err" >/dev/null || fail "spawn did not explain missing bin"

  printf 'domain\n' > "$home/.sc-secondmate-home"
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$home" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted the active home"
  fi
  grep -F 'secondmate home cannot be the active souschef home' "$err" >/dev/null || fail "spawn did not reject active home"

  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$ROOT" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted the souschef repo root"
  fi
  grep -F 'secondmate home cannot be the souschef repo' "$err" >/dev/null || fail "spawn did not reject souschef repo root"

  printf 'domain\n' > "$active_descendant/.sc-secondmate-home"
  printf 'charter\n' > "$active_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$active_descendant" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home inside the active souschef home"
  fi
  grep -F 'secondmate home cannot be inside the active souschef home' "$err" >/dev/null || fail "spawn did not reject active home descendant"

  printf 'domain\n' > "$active_ancestor/.sc-secondmate-home"
  printf 'charter\n' > "$active_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" SC_HOME="$ancestor_active_home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$active_ancestor" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home containing the active souschef home"
  fi
  grep -F 'secondmate home cannot be an ancestor of the active souschef home' "$err" >/dev/null || fail "spawn did not reject active home ancestor"

  printf 'domain\n' > "$root_descendant/.sc-secondmate-home"
  printf 'charter\n' > "$root_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" SC_ROOT_OVERRIDE="$fakeroot" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$root_descendant" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home inside the souschef repo"
  fi
  grep -F 'secondmate home cannot be inside the souschef repo' "$err" >/dev/null || fail "spawn did not reject repo root descendant"

  printf 'domain\n' > "$root_ancestor/.sc-secondmate-home"
  printf 'charter\n' > "$root_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" SC_ROOT_OVERRIDE="$root_inside" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/sc-spawn.sh" domain "$root_ancestor" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home containing the souschef repo"
  fi
  grep -F 'secondmate home cannot be an ancestor of the souschef repo' "$err" >/dev/null || fail "spawn did not reject repo ancestor"
...
test_secondmate_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  for opdir in data state config projects; do
    ...
    : > "$log"
    if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-opdir-fake/pane.txt" \
      "$ROOT/bin/sc-spawn.sh" domain "$subhome" codex --secondmate >/dev/null 2>"$err"; then
...
test_fm_send_refuses_bare_window_without_home_meta() {
  # The happy path (a bare sc-<id> resolves the window recorded in THIS home's
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_WINDOW="other-session:sc-missing" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/send-fake/pane.txt" \
    "$ROOT/bin/sc-send.sh" sc-missing 'wrong home' >/dev/null 2>"$err"; then
    fail "sc-send sent to a bare souschef window without home metadata"
  fi
  grep -F "no metadata for sc-missing in $home/state" "$err" >/dev/null \
    || fail "sc-send did not explain missing home metadata"
  grep -F 'send-keys -t other-session:sc-missing' "$log" >/dev/null \
    && fail "sc-send fell back to a foreign same-name window"
  pass "sc-send refuses a bare souschef window with no metadata in this home"
}
...
test_secondmate_force_teardown_discards_child_work() {
  ...
  mkdir -p "$home/state" "$home/data" "$subhome/state"
  sc_git_worktree "$childproj" "$childwt" force-child
  printf 'domain\n' > "$subhome/.sc-secondmate-home"
  cat > "$home/state/domain.meta" <<EOF
window=souschef:sc-domain
worktree=$subhome
...
EOF
  ...
  grep -F 'kill-window -t souschef:sc-child' "$log" >/dev/null || fail "force teardown did not kill child window"
  grep -F 'kill-window -t souschef:sc-domain' "$log" >/dev/null || fail "force teardown did not kill parent window"
...
EOF
    ...
    PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-inside-teardown-fake-$opdir/pane.txt" \
      "$ROOT/bin/sc-teardown.sh" domain --force >/dev/null 2>"$err" \
    ...
    grep -F 'kill-window -t souschef:sc-domain' "$log" >/dev/null || fail "force teardown did not kill parent window for inside $opdir symlink"
  done
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-state-teardown-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
test_secondmate_teardown_path_boundary_matrix() {
  ...
  # an ancestor of the active souschef home, inside the active souschef home,
  # or inside the souschef repo. One row per hazard, one shared assertion block.
  local row base home subhome fmroot fakebin log err expect tid
  while IFS='|' read -r row expect; do
    [ -n "$row" ] || continue
    base="$TMP_ROOT/td-pb-$row"
    fmroot="$ROOT"   # real souschef repo unless a row overrides it
    tid=domain
    case "$row" in
      unmarked)
        home="$base/main"; subhome="$base/sub"
        mkdir -p "$home/state" "$home/data" "$subhome/state"
        # No .sc-secondmate-home marker on purpose.
        ;;
      ancestor)
        # The home being torn down is an ANCESTOR of the active souschef home.
      ...
      repo-descendant)
        ...
        cat > "$fmroot/bin/sc-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
        chmod +x "$fmroot/bin/sc-guard.sh"
        printf 'repo-domain\n' > "$subhome/.sc-secondmate-home"
        ;;
    esac
    sc_write_secondmate_meta "$home/state/$tid.meta" "$subhome"
    ...
    if PATH="$fakebin:$PATH" SC_ROOT_OVERRIDE="$fmroot" SC_HOME="$home" \
      SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$base/fake/pane.txt" \
      "$ROOT/bin/sc-teardown.sh" "$tid" >/dev/null 2>"$err"; then
...
ancestor|ancestor of the active souschef home
active-descendant|inside the active souschef home
repo-descendant|inside the souschef repo
ROWS
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/nested-teardown-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain >/dev/null 2>"$err"; then
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-registry-teardown-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain >/dev/null 2>"$err"; then
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/prevalidate-teardown-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-active-descendant-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside active SC_HOME"
  fi
  ...
  grep -F 'inside the active souschef home' "$err" >/dev/null || fail "force teardown did not explain active home descendant rejection"
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_ROOT_OVERRIDE="$fakeroot" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-repo-descendant-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside SC_ROOT"
  fi
  ...
  grep -F 'inside the souschef repo' "$err" >/dev/null || fail "force teardown did not explain repo descendant rejection"
  pass "force teardown refuses child worktrees inside the souschef repo"
}
...
EOF
  ...
  if PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_LOG="$log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/unregistered-child-fake/pane.txt" \
    "$ROOT/bin/sc-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
EOF
  ...
  PATH="$fakebin:$PATH" SC_HOME="$home" SC_FAKE_TMUX_WINDOW="$window" SC_FAKE_TMUX_LOG="$TMP_ROOT/watch-fake/tmux.log" SC_FAKE_TMUX_CAPTURE="$TMP_ROOT/watch-fake/pane.txt" \
    SC_POLL=1 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$ROOT/bin/sc-watch.sh" > "$out" &
  pid=$!
...
test_secondmate_charter_brief_is_idle_by_default() {
  ...
  grep -F 'go idle and wait silently for the main souschef' "$brief" >/dev/null \
    || fail "charter brief does not tell the secondmate to go idle and wait for routed work"
  grep -F 'Act only on tasks the main souschef routes to you' "$brief" >/dev/null \
  ...
  grep -F 'run normal souschef bootstrap and recovery' "$brief" >/dev/null \
...
EOF
  ...
  if SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" design bug-z no-such-key >/dev/null 2>&1; then
  ...
  before=$(cat "$home/data/backlog.md")
  if SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" design live-task >/dev/null 2>&1; then
  ...
  if SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" ghost bug-z >/dev/null 2>&1; then
    fail "handoff accepted an unregistered secondmate id"
  fi
  pass "sc-backlog-handoff aborts atomically on unmatched, in-flight, and unregistered targets"
}
...
EOF
  SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" archive shipped-task >/dev/null \
  ...
  if SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" proj-sm shipped-task >/dev/null 2>&1; then
...
EOF
  if SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" marker-sm marker-task >/dev/null 2>&1; then
...
EOF
  if SC_HOME="$home" "$ROOT/bin/sc-backlog-handoff.sh" symlink-sm symlink-task >/dev/null 2>&1; then
  ...
  pass "sc-backlog-handoff creates absent sections and refuses unsafe homes"
}
