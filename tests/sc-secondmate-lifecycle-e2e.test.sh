#!/usr/bin/env bash
# tests/sc-secondmate-lifecycle-e2e.test.sh - the happy-path secondmate operator
...
# lease-specific paths live in sc-secondmate-safety.test.sh.
#
...
#   - a bare `sc-<id>` send targets the window recorded in THIS home's meta
...
TMP_ROOT=$(sc_test_tmproot sc-secondmate-lifecycle)
...
setup_world() {
  ...
  sc_git_init_commit "$HOME_DIR/projects/alpha"
  sc_git_init_commit "$HOME_DIR/projects/beta"
  sc_git_init_commit "$HOME_DIR/projects/gamma"
  sc_git_add_origin "$HOME_DIR/projects/alpha" "$TMP_ROOT/remotes/alpha.git"
  sc_git_add_origin "$HOME_DIR/projects/beta" "$TMP_ROOT/remotes/beta.git"
  sc_git_add_origin "$HOME_DIR/projects/gamma" "$TMP_ROOT/remotes/gamma.git"
...
EOF
  ...
  # One combined fakebin: tmux + sc-worktree (spawn/send/teardown).
  ...
  SC_SECONDMATE_SCOPE='customer onboarding from brief' \
...
phase_seed() {
  local out
  out=$(PATH="$FAKEBIN:$PATH" SC_HOME="$HOME_DIR" \
    "$ROOT/bin/sc-home-seed.sh" design "$SUB" alpha beta gamma) \
    || fail "seed failed"
  SUB_ABS=$(cd "$SUB" && pwd -P)

  assert_contains "$out" "home=$SUB_ABS" "seed did not report the subhome"
  assert_present "$SUB/.sc-secondmate-home" "seed did not mark the subhome"
  ...
  [ "$(SC_HOME="$SUB" "$ROOT/bin/sc-project-mode.sh" alpha)" = "direct-PR on" ] \
    || fail "alpha delivery mode not preserved in the subhome"
  [ "$(SC_HOME="$SUB" "$ROOT/bin/sc-project-mode.sh" beta)" = "direct-PR off" ] \
    || fail "beta delivery mode not preserved in the subhome"
  SC_HOME="$HOME_DIR" "$ROOT/bin/sc-home-seed.sh" validate >/dev/null || fail "registry validation failed after seed"

  pass "seed: registry scope+projects, charter copied, clones+origins preserved, parent untouched"
}

phase_spawn() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" SC_HOME="$HOME_DIR" SC_CONFIG_OVERRIDE="$HOME_DIR/parent-config" \
    SC_FAKE_TMUX_LOG="$LOG" SC_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/sc-spawn.sh" design "$SUB" codex --secondmate >/dev/null \
...
phase_send() {
  : > "$LOG"
  # The meta window (souschef:sc-design) must win over a foreign same-named
  # window returned by list-windows.
  PATH="$FAKEBIN:$PATH" SC_HOME="$HOME_DIR" SC_FAKE_TMUX_WINDOW="other-session:sc-design" \
    SC_FAKE_TMUX_LOG="$LOG" SC_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/sc-send.sh" sc-design 'route this work' >/dev/null 2>&1 \
    || fail "sc-send failed for a bare souschef window with home metadata"
  # design is a kind=secondmate target, so the request is prefixed with the
  # from-souschef marker (bin/sc-marker-lib.sh): the send targets the meta window
  # AND carries the marker label, and the original payload still follows it.
  assert_grep 'send-keys -t souschef:sc-design -l [sc-from-souschef]' "$LOG" "send did not use the window recorded in this home's meta, or did not mark the secondmate request"
  assert_grep 'route this work' "$LOG" "the original request text did not survive the marker"
  assert_no_grep 'send-keys -t other-session:sc-design' "$LOG" "send targeted a foreign same-named window"
  pass "send: a bare sc-<id> secondmate routes to the meta window with the from-souschef marker"
}
...
EOF
  local out before
  out=$(SC_HOME="$HOME_DIR" "$ROOT/bin/sc-backlog-handoff.sh" design feat-x feat-y) \
  ...
  SC_HOME="$HOME_DIR" "$ROOT/bin/sc-backlog-handoff.sh" design feat-x feat-y >/dev/null 2>&1 \
...
phase_recovery() {
  ...
  PATH="$FAKEBIN:$PATH" SC_HOME="$HOME_DIR" SC_FAKE_TMUX_LOG="$LOG" SC_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/sc-spawn.sh" design "echo relaunch" --secondmate >/dev/null 2>&1 \
  ...
  assert_grep 'window=souschef:sc-design' "$meta" "respawn did not reconstruct the direct-report window"
...
phase_teardown() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" SC_HOME="$HOME_DIR" SC_FAKE_TMUX_LOG="$LOG" SC_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/sc-teardown.sh" design >/dev/null 2>&1 \
