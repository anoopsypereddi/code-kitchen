...
# real OS-level desktop notification by default. Point its SC_WEDGE_ALARM_EXEC
# seam at a recorder for every daemon/wake suite, so no test - present or future
# - can post a real macOS, herdr, or command: notification: it is impossible to
# forget, because sourcing this harness installs it. The recorder is an on-disk
# script (a real daemon a test spawns inherits the path and records too). It logs
# "<channel>\t<summary>" to $SC_WEDGE_ALARM_LOG, which a test sets to its own file
# to assert on; unset means /dev/null. SC_WEDGE_ALARM_FAIL=<channel> makes the
# recorder exit non-zero for that channel, to exercise graceful degradation.
# Suites that do not source this harness still cannot fire a real notification:
# the daemon defaults the seam to "discard" whenever it is sourced (its
# library-mode guard). Create the recorder dir with mktemp directly (not
# sc_test_tmproot, whose first call installs an EXIT trap that, invoked inside a
# command-substitution subshell, would delete the dir on subshell exit). Register
# it for the same cleanup and install the trap here if it is the first one.
_sc_wedge_rec_dir=$(mktemp -d "${TMPDIR:-/tmp}/sc-wedge-rec.XXXXXX")
if [ "${#SC_TEST_CLEANUP_DIRS[@]}" -eq 0 ]; then trap sc_test_cleanup EXIT; fi
SC_TEST_CLEANUP_DIRS+=("$_sc_wedge_rec_dir")
cat > "$_sc_wedge_rec_dir/rec" <<'REC'
#!/usr/bin/env bash
printf '%s\t%s\n' "${1:-}" "${2:-}" >> "${SC_WEDGE_ALARM_LOG:-/dev/null}"
case " ${SC_WEDGE_ALARM_FAIL:-} " in *" ${1:-} "*) exit 1 ;; esac
exit 0
REC
chmod +x "$_sc_wedge_rec_dir/rec"
export SC_WEDGE_ALARM_EXEC="$_sc_wedge_rec_dir/rec"
