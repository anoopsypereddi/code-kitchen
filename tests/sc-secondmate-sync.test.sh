...
# PRIMARY souschef checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/sc-spawn.sh
# (before launching a secondmate) and bin/sc-bootstrap.sh (a startup sweep of
...
TMP_ROOT=$(sc_test_tmproot sc-secondmate-sync)

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY souschef repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh watcher beacon keeps sc-guard quiet for the spawn path.
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  printf 'sm-nonlive\n' > "$w/sm-nonlive/.sc-secondmate-home"

  fakebin=$(make_fake_toolchain "$w")
  out=$(PATH="$fakebin:$BASE_PATH" SC_HOME="$w/home" SC_ROOT_OVERRIDE="$w/main" \
    "$ROOT/bin/sc-bootstrap.sh" 2>/dev/null)

  nudge_line=$(printf '%s\n' "$out" | grep '^NUDGE_SECONDMATES:' || true)
  [ -n "$nudge_line" ] || fail "no NUDGE_SECONDMATES line emitted (got: $out)"
  assert_contains "$nudge_line" "souschef:sc-sm-instr" "instruction-changed running secondmate is nudged"
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    SC_ROOT_OVERRIDE="$w/main" SC_HOME="$w/home" \
    SC_STATE_OVERRIDE="$w/home/state" SC_DATA_OVERRIDE="$w/home/data" \
    SC_PROJECTS_OVERRIDE="$w/home/projects" SC_CONFIG_OVERRIDE="$w/home/config" \
    SC_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/sc-spawn.sh" sm "$w/sm" codex --secondmate >/dev/null 2>&1 || true
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    SC_PROJECTS_OVERRIDE="$w/home/projects" SC_CONFIG_OVERRIDE="$w/home/config" \
    SC_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/sc-spawn.sh" sm "$w/sm" codex --secondmate >/dev/null 2>"$err" || true
...
echo "# all sc-secondmate-sync tests passed"
