#!/usr/bin/env bash
# tests/sc-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
LIB="$ROOT/bin/sc-wake-lib.sh"

TMP_ROOT=$(sc_test_tmproot sc-watcher-lock-tests)
...
test_singleton_start() {
  ...
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$state" SC_POLL=5 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH" > "$out1" &
  pid1=$!
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$state" SC_POLL=5 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH" > "$out2" &
  pid2=$!
  sleep 0.5
  live=0
...
test_stale_watch_lock_reclaimed() {
  ...
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$state" SC_POLL=5 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  sleep 0.5
  live=0
...
test_guard_warnings() {
  ...
  # SC_ROOT_OVERRIDE points the worktree-tangle check at a non-git dir so it stays
  ...
  SC_ROOT_OVERRIDE="$dir" SC_STATE_OVERRIDE="$state" SC_GUARD_GRACE=1 "$ROOT/bin/sc-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  grep -F 'bin/sc-watch-arm.sh' "$err" >/dev/null || fail "guard banner missing the fix command"
  ...
  # Non-git SC_ROOT keeps the worktree-tangle check inert so "fresh watcher ->
  # total silence" stays a pure assertion about watcher state.
  SC_ROOT_OVERRIDE="$dir" SC_STATE_OVERRIDE="$state" SC_GUARD_GRACE=300 "$ROOT/bin/sc-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
...
test_lock_single_winner_under_concurrency() {
  ...
  pass "concurrent sc_lock_try_acquire yields exactly one winner"
}
...
test_lock_steals_dead_pid_lock() {
  ...
  printf '%s\n' "$dead" > "$lockdir/pid"
  rc=0
  newpid=$(SC_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if sc_lock_try_acquire "$2"; then cat "$2/pid"; else exit 7; fi
  ' _ "$LIB" "$lockdir") || rc=$?
...
test_lock_live_steal_mutex_is_not_reclaimed() {
  ...
  SC_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    sc_lock_try_acquire "$2.steal" || exit 7
    printf "%s\n" "${BASHPID:-$$}" > "$3"
    sleep 2
    sc_lock_release "$2.steal"
  ...
  out=$(SC_LOCK_STALE_AFTER=0 SC_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if sc_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s lockpid=%s stealpid=%s\n" "$rc" "${SC_LOCK_HELD_PID:-}" "$(cat "$2/pid" 2>/dev/null || true)" "$(cat "$2.steal/pid" 2>/dev/null || true)"
  ' _ "$LIB" "$lockdir")
...
test_lock_does_not_steal_live_lock() {
  ...
  mkdir "$lockdir"
  printf '%s\n' "$live" > "$lockdir/pid"
  out=$(SC_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if sc_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s\n" "$rc" "${SC_LOCK_HELD_PID:-}"
  ' _ "$LIB" "$lockdir")
  ...
  case "$out" in
    *"held=$live"*) ;;
    *) fail "live holder pid not reported via SC_LOCK_HELD_PID: $out" ;;
  esac
...
test_lock_late_claim_loses_after_recreate() {
  ...
  out=$(SC_LOCK_STALE_AFTER=0 SC_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner1=$(sc_lock_owner_dir "$2") || exit 20
    ln -s "$owner1" "$2" || exit 21
    touch -h -t 200001010000 "$2" 2>/dev/null || sleep 2
    if ! sc_lock_try_acquire "$2"; then exit 22; fi
    before=$(cat "$2/pid" 2>/dev/null || true)
    if sc_lock_claim "$2" "$owner1"; then late=won; else late=lost; fi
...
test_lock_paused_mid_acquire_claim_fails_during_steal() {
  ...
  out=$(SC_LOCK_STALE_AFTER=0 SC_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner=$(sc_lock_owner_dir "$2") || exit 20
    ln -s "$owner" "$2" || exit 21
    sc_lock_try_acquire "$2.steal" || exit 22
    steal_owner=${SC_LOCK_OWNER_DIR:-}
    if sc_lock_claim "$2" "$owner"; then late=won; else late=lost; fi
    if sc_lock_try_create "$2" "$steal_owner"; then stealer=won; else stealer=lost; fi
...
test_watch_restart_rejects_reused_pid() {
  ...
  mkdir "$state/.watch.lock"
  printf '%s\n' "$live" > "$state/.watch.lock/pid"
  printf '%s\n' "$dir" > "$state/.watch.lock/sc-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "stale watcher identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" SC_HOME="$dir" SC_POLL=5 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH_ARM" --restart > "$out" &
  pid=$!
...
test_watcher_self_evicts_on_lock_takeover() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$state" SC_POLL=1 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  while [ "$i" -lt 50 ]; do
...
test_arm_reports_healthy_for_live_fresh_watcher() {
  ...
  status=0
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$state" "$WATCH_ARM" > "$armout" || status=$?
...
test_arm_starts_and_self_heals() {
  ...
  for row in clean dead-pid; do
    ...
    PATH="$fakebin:$PATH" SC_HOME="$dir" SC_POLL=5 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    armpid=$!
    i=0
    while [ "$i" -lt 80 ]; do
...
SH
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after immediate arm wake failed"
...
test_arm_waits_for_peer_beacon_after_child_stands_down() {
  ...
  sleep 300 &
  peer=$!
  identity=$(SC_STATE_OVERRIDE="$state" bash -c '. "$1"; sc_pid_identity "$2"' _ "$LIB" "$peer") || fail "could not identify peer pid"
  mkdir "$state/.watch.lock"
  ...
  beater=$!
  status=0
  PATH="$fakebin:$PATH" SC_HOME="$dir" SC_POLL=5 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 SC_ARM_CONFIRM_TIMEOUT=4 "$WATCH_ARM" > "$armout" || status=$?
