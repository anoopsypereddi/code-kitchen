#!/usr/bin/env bash
# tests/sc-wake-daemon-lifecycle-e2e.test.sh - the watcher + supervise-daemon
...
# sc-watch.sh runs plus the daemon's own functions. The captain-relevant
# status-phrase matrix and the lock-primitive races stay as focused units
# (sc-daemon.test.sh, sc-watcher-lock.test.sh) - an e2e cannot deterministically
...
TMP_ROOT=$(sc_test_tmproot sc-wake-daemon-e2e)

# Run the one-shot watcher once: it exits with a single reason line when a wake
# fires. fakebin shadows tmux. Echoes nothing; the caller reads $out.
run_watcher_once() {
  local state=$1 fakebin=$2 out=$3
  PATH="$fakebin:$PATH" SC_STATE_OVERRIDE="$state" SC_POLL=1 SC_SIGNAL_GRACE=1 \
    SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH" > "$out" &
  wait_for_exit "$!" 50
}
...
test_routine_then_terminal_after_restart() {
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after routine signal failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
    || fail "routine signal was not queued"
  SC_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  ...
  : > "$drain_out"
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after terminal signal failed"
  SC_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  ...
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_PANE_ALIVE=1 SC_FAKE_TMUX_SENT="$sent" \
    SC_FAKE_TMUX_CAPTURE="$dir/pane.txt" SC_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_stale_pane_transient_persistent_resume() {
  ...
  case "$(SC_STATE_OVERRIDE="$state" classify_stale "$win" "$state")" in
    self\|*) : ;;
  ...
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_WINDOW="$win" SC_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
