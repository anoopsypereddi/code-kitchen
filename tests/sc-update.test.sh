#!/usr/bin/env bash
# Tests for bin/sc-update.sh: fast-forward-only self-update of a running
# souschef repo and every registered secondmate home.
#
# The guarantees under test mirror sc-fleet-sync.sh and prime directive #3:
#   - The running souschef repo (on its default branch) fast-forwards from
...
#   - The caller-action summary is correct: reread-souschef flips to yes only
...
#     data/secondmates.md registry, deduped, and the souschef repo is never
...
UPDATE="$ROOT/bin/sc-update.sh"

# Deterministic, isolated git identity for fixture commits.
sc_git_identity fmtest fmtest@example.com

TMP_ROOT=$(sc_test_tmproot sc-update-tests)

# Build a fresh world: a bare origin seeded with one commit, a souschef repo
...
new_world() {
  ...
  # Fresh watcher beacon keeps sc-guard quiet.
...
add_sm() {
  ...
  {
    printf 'window=main:sc-%s\n' "$id"
    printf 'kind=secondmate\n'
    printf 'home=%s/%s\n' "$w" "$id"
  } > "$w/home/state/$id.meta"
  printf '%s\n' "$id" > "$w/$id/.sc-secondmate-home"
}
...
run_update() {
  local w=$1
  SC_ROOT_OVERRIDE="$w/main" SC_HOME="$w/home" "$UPDATE" 2>/dev/null
}
...
test_updates_main_and_secondmate() {
  ...
  assert_contains "$out" "souschef: updated " "souschef fast-forwarded"
  assert_contains "$out" "secondmate sm1: updated " "secondmate fast-forwarded"
  assert_contains "$out" "reread-souschef: yes" "instruction change triggers reread"
  assert_contains "$out" "nudge-secondmates: main:sc-sm1" "updated secondmate is nudged"

  # Fast-forward landed: HEAD == origin/main on both targets.
  [ "$(git -C "$w/main" rev-parse HEAD)" = "$(git -C "$w/main" rev-parse origin/main)" ] \
    || fail "souschef HEAD not at origin/main"
  [ "$(git -C "$w/sm1" rev-parse HEAD)" = "$(git -C "$w/sm1" rev-parse origin/main)" ] \
    || fail "secondmate HEAD not at origin/main"
  # Souschef stays on its default branch; secondmate stays detached.
  [ "$(git -C "$w/main" symbolic-ref --short HEAD 2>/dev/null)" = "main" ] \
    || fail "souschef left its default branch"
  ...
  [ "$(git -C "$w/main" rev-list --parents -n1 HEAD | wc -w | tr -d ' ')" -eq 2 ] \
    || fail "souschef tip is not a single-parent fast-forward"
...
test_reread_gate_is_instruction_only() {
  ...
  assert_contains "$out" "souschef: updated " "souschef still advanced"
  assert_contains "$out" "reread-souschef: no" "non-instruction change skips reread"
  # The secondmate still advanced, so it is still nudged (update-based nudge).
  assert_contains "$out" "nudge-secondmates: main:sc-sm1" "advanced secondmate still nudged"
...
test_dirty_secondmate_skipped() {
  ...
  assert_not_contains "$out" "sc-sm1" "skipped secondmate is not nudged"
...
test_diverged_secondmate_skipped() {
  ...
  assert_not_contains "$out" "sc-sm1" "diverged secondmate is not nudged"
...
test_idempotent_already_current() {
  ...
  assert_contains "$out" "souschef: already current" "souschef already current"
  assert_contains "$out" "secondmate sm1: already current" "secondmate already current"
  assert_contains "$out" "reread-souschef: no" "no reread when nothing changed"
...
#   selfish - a bogus registry line pointing the souschef repo at itself.
# Asserts: reg1 advances but is NOT nudged (no live metadata); sm1 advances,
# is processed once, and IS nudged; the souschef repo is never re-processed.
test_registry_backstop_dedup_and_self_exclusion() {
  local w out count
  w=$(new_world t7)
  add_sm "$w" sm1
  git -C "$w/main" worktree add -q --detach "$w/reg1" main
  printf 'reg1\n' > "$w/reg1/.sc-secondmate-home"
  {
  ...
  assert_not_contains "$out" "secondmate selfish" "souschef repo re-processed as its own secondmate"
  ...
  assert_contains "$nudge_line" "main:sc-sm1" "live-meta secondmate is nudged"
  assert_not_contains "$nudge_line" "reg1" "registry-only secondmate without live metadata is not nudged"
  pass "T7 registry backstop resolves, dedups meta+registry, excludes the souschef repo"
}

# --- T9: souschef repo on a feature branch is skipped ---------------------
test_souschef_wrong_branch_skipped() {
  local w out before
  w=$(new_world t9)
  bump_origin "$w" instr
  # Simulate souschef mid-shipping its own change: not on the default branch.
  git -C "$w/main" checkout -q -b feature/wip
  before=$(git -C "$w/main" rev-parse HEAD)

  out=$(run_update "$w")

  assert_contains "$out" "souschef: skipped: on feature/wip, expected main" "off-default souschef skipped"
  assert_contains "$out" "reread-souschef: no" "no reread when souschef was skipped"
  [ "$(git -C "$w/main" rev-parse HEAD)" = "$before" ] \
    || fail "skipped souschef HEAD moved"
  pass "T9 souschef off its default branch is skipped, not forced"
}

test_souschef_detached_head_skipped() {
  local w out before
  w=$(new_world t10)
  bump_origin "$w" instr
  ...
  assert_contains "$out" "souschef: skipped: detached HEAD, expected main" "detached souschef skipped"
  assert_contains "$out" "reread-souschef: no" "no reread when detached souschef was skipped"
  [ "$(git -C "$w/main" rev-parse HEAD)" = "$before" ] \
    || fail "detached souschef HEAD moved"
  pass "T10 souschef detached HEAD is skipped"
}
...
test_unsafe_secondmate_home_skipped_before_git_update() {
  ...
  assert_contains "$out" "secondmate bad: skipped: unsafe home: secondmate home cannot be inside the active souschef home" \
...
echo "# all sc-update tests passed"
