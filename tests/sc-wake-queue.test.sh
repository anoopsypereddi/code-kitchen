#!/usr/bin/env bash
# tests/sc-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# lives in sc-watcher-lock.test.sh; daemon classification/injection in
# sc-daemon.test.sh.
set -u
...
TMP_ROOT=$(sc_test_tmproot sc-wake-tests)
...
test_concurrent_append_and_drain() {
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pids="$pids $!"
  for pid in $pids; do
    wait "$pid" || fail "concurrent append/drain subprocess failed"
  done
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" || fail "final drain failed"
  cat "$out1" "$out2" > "$all"
...
test_signal_catchup_without_running_watcher() {
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after first signal failed"
...
test_stale_enqueue_before_suppressor() {
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" SC_FAKE_TMUX_WINDOW="$window" SC_FAKE_TMUX_CAPTURE="$capture_file" SC_STATE_OVERRIDE="$state" SC_POLL=1 SC_SIGNAL_GRACE=1 SC_CHECK_INTERVAL=999999 SC_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after stale wake failed"
...
SH
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after check wake failed"
...
test_atomic_double_drain() {
  ...
  append_wake "$state" stale 's:sc-task' 'stale: s:sc-task' || fail "stale append failed"
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pid1=$!
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" &
  pid2=$!
  ...
  leftover=$(SC_STATE_OVERRIDE="$state" "$DRAIN" | awk 'NF { count++ } END { print count + 0 }')
...
test_drain_dedupes_obvious_duplicates() {
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "dedupe drain failed"
...
# watcher liveness via sc-guard.sh: a lapsed re-arm chain then surfaces even on a
...
test_drain_asserts_watcher_liveness() {
  ...
  SC_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || fail "drain failed while asserting liveness"
  ...
  SC_STATE_OVERRIDE="$state" SC_GUARD_GRACE=300 "$DRAIN" >/dev/null 2> "$err" || fail "drain failed with a fresh beacon"
