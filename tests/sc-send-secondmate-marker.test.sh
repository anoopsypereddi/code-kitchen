#!/usr/bin/env bash
# sc-send from-souschef marker for secondmate targets.
#
# A secondmate is itself a souschef, so a request relayed to it lands in its own
# chat - which the main souschef never reads (the only channel back is the terse
# status file). sc-send therefore prepends a from-souschef marker
# (bin/sc-marker-lib.sh) when, and only when, the resolved target is a bare
# `sc-<id>` whose meta records kind=secondmate, so the secondmate can recognize
...
#   5. The marker is exactly the label "[sc-from-souschef]" + ASCII 0x1f, and the
#      sc_message_from_souschef detector keys on that untypable sequence.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"
# shellcheck source=bin/sc-marker-lib.sh
. "$ROOT/bin/sc-marker-lib.sh"

SEND="$ROOT/bin/sc-send.sh"

TMP_ROOT=$(sc_test_tmproot sc-send-marker)

# A fake tmux that (a) records the literal text of every `send-keys -l` to
# SC_SEND_LOG and (b) lets sc-send's submit path reach a clean "empty" verdict.
# display-message yields a numeric cursor_y; capture-pane returns an empty
# bordered composer so sc_tmux_composer_state reads "empty" (submit landed) on the
...
# run_send <fakebin> <home> <send-log> -- <sc-send args...>
# Runs sc-send.sh with the stubs on PATH against the given home (which holds
# state/<id>.meta). SC_ROOT_OVERRIDE points at the same non-repo home so
# sc-guard's tangle check stays silent; guard noise goes to stderr (discarded).
# SC_SEND_SETTLE=0 keeps the run fast. Truncates the log first; returns sc-send's
# exit code.
run_send() {
  local fb=$1 home=$2 log=$3; shift 3
  : > "$log"
  env PATH="$fb:$PATH" \
    SC_ROOT_OVERRIDE="$home" SC_HOME="$home" SC_SEND_LOG="$log" SC_SEND_SETTLE=0 \
    "$SEND" "$@" 2>/dev/null
}
...
test_secondmate_target_is_marked() {
  ...
  home=$(setup_home sm)
  sc_write_secondmate_meta "$home/state/domain.meta" "$home" "sess:sc-domain"
  run_send "$fb" "$home" "$log" "sc-domain" "audit the build"; rc=$?
  expect_code 0 "$rc" "send to a secondmate target should succeed"
  got=$(cat "$log")
  case "$got" in
    "$SC_FROMFIRST_MARK"audit\ the\ build) : ;;
    *) fail "secondmate send: literal text should be marker+text"$'\n'"--- bytes ---"$'\n'"$(printf '%s' "$got" | od -An -c)" ;;
  esac
  pass "sc-send: a kind=secondmate target gets the from-souschef marker prepended"
}
...
test_crewmate_target_is_not_marked() {
  ...
  sc_write_meta "$home/state/build.meta" \
    "window=sess:sc-build" "worktree=$home/wt" "project=$home/p" \
    "harness=echo" "kind=ship" "mode=no-mistakes" "yolo=off"
  run_send "$fb" "$home" "$log" "sc-build" "fix the test"; rc=$?
  expect_code 0 "$rc" "send to a crewmate target should succeed"
  got=$(cat "$log")
  [ "$got" = "fix the test" ] \
    || fail "crewmate send: expected bare text, got marker or other"$'\n'"--- bytes ---"$'\n'"$(printf '%s' "$got" | od -An -c)"
  pass "sc-send: a kind=ship (crewmate) target is sent unmarked"
}
...
test_explicit_window_is_not_marked() {
  ...
  sc_write_secondmate_meta "$home/state/win.meta" "$home" "other:win"
  ...
  pass "sc-send: an explicit session:window target is never marked"
}
...
test_key_path_is_not_marked() {
  ...
  home=$(setup_home key)
  sc_write_secondmate_meta "$home/state/domain.meta" "$home" "sess:sc-domain"
  run_send "$fb" "$home" "$log" "sc-domain" --key Escape; rc=$?
