...
TEARDOWN="$ROOT/bin/sc-teardown.sh"
PR_CHECK="$ROOT/bin/sc-pr-check.sh"
TMP_ROOT=$(sc_test_tmproot sc-teardown-tests)
...
add_fork_with_pushed_branch() {
  ...
  # so refs/remotes/fork/sc-task-x1 is visible from the worktree (shared object db).
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "sc-pr-check does not refresh PR head after HEAD moves"
}
