...
# is well. souschef consumes the exact 'MISSING: treehouse (install: ...)' and
...
# treehouse's `get --help` advertises --lease only when SC_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${SC_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks mode expect notcontains; do
    ...
    out=$(PATH="$fakebin:$BASE_PATH" SC_HOME="$case_dir/home" SC_ROOT_OVERRIDE="$case_dir/home" \
      SC_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/sc-bootstrap.sh")
    case "$mode" in
      empty)
