#!/usr/bin/env bash
# tests/sc-afk-inject-e2e.test.sh - private-socket end-to-end test for the afk
...
# private socket. The daemon points at a throwaway state dir (SC_STATE_OVERRIDE)
# and the test pane (SC_SUPERVISOR_TARGET). Nothing touches the live fleet.
#
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sc-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
LOG_FILE="$STATE_DIR/submitted.log"
: > "$LOG_FILE"

# Source the daemon to get SC_INJECT_MARK, afk_enter, afk_exit.
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sc-shim.XXXXXX")
...
# Create a fake crewmate window (the watcher lists sc-* windows for stale
# detection). The pane is an inert shell — it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n sc-fake-c1 -t supervisor

start_daemon() {
  PATH="$TMUX_SHIM_DIR:$PATH" \
  SC_STATE_OVERRIDE="$STATE_DIR" \
  SC_SUPERVISOR_TARGET="$SUPERVISOR_PANE" \
  SC_ESCALATE_BATCH_SECS=0 \
  SC_HOUSEKEEPING_TICK=1 \
  SC_POLL=1 \
  SC_SIGNAL_GRACE=1 \
  SC_HEARTBEAT=999999 \
  SC_CHECK_INTERVAL=999999 \
  SC_INJECT_CONFIRM_SLEEP=0.3 \
  SC_INJECT_CONFIRM_RETRIES=5 \
  SC_STALE_ESCALATE_SECS=999999 \
