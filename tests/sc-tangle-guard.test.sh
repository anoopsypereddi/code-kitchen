...
#            sc-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - sc-guard and sc-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the sc-guard banner,
# the sc-bootstrap problem line, the brief assertion ordering, and the sc-spawn
...
. "$ROOT/bin/sc-tangle-lib.sh"

TMP_ROOT=$(sc_test_tmproot sc-tangle-guard)
...
# sc_primary_tangle_branch is the whole scoping decision: a NAMED non-default
...
test_lib_classification() {
  ...
  while IFS='|' read -r label state branch expect; do
    ...
    out=$(sc_primary_tangle_branch "$repo" || true)
...
ROWS
  ...
  out=$(sc_primary_tangle_branch "$TMP_ROOT" || true)
  [ -z "$out" ] || fail "non-git dir wrongly reported a tangle: '$out'"
  pass "sc_primary_tangle_branch: feature branch alarms; default/detached/non-git stay silent"
}

# --- GUARD 2a: sc-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  SC_ROOT_OVERRIDE="$1" SC_HOME="$1" "$ROOT/bin/sc-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "sc-guard: bordered tangle banner fires only for a feature branch in the primary"
}

# --- GUARD 2b: sc-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps fleet sync inert; grep isolates the line.
  SC_ROOT_OVERRIDE="$1" SC_HOME="$1" "$ROOT/bin/sc-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  pass "sc-bootstrap: TANGLE problem line fires only for a feature branch in the primary"
}
...
test_brief_assertion_precedes_branch() {
  ...
  mkdir -p "$home/data"
  SC_HOME="$home" "$ROOT/bin/sc-brief.sh" tangle-brief-cc3 alpha >/dev/null 2>&1
  ...
  pass "sc-brief: ship brief asserts worktree isolation before the branch step"
}
...
test_spawn_isolation_abort() {
  ...
  pass "sc-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}
