...
#   1. sc_tmux_strip_ghost drops dim/faint runs and keeps normal-intensity text.
#   2. sc_pane_input_pending reads a dim-ghost-only composer as NOT pending, while
#      still treating real (normal-intensity) text as pending.
#   3. The human/LLM-facing capture path (sc-peek.sh) stays PLAIN - no escape codes
#      ever reach souschef's context.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LIB="$ROOT/bin/sc-tmux-lib.sh"
PEEK="$ROOT/bin/sc-peek.sh"

# shellcheck source=bin/sc-tmux-lib.sh
. "$LIB"

TMP_ROOT=$(sc_test_tmproot sc-ghost-tests)
...
test_strip_ghost_drops_dim_keeps_normal() {
  ...
  pass "sc_tmux_strip_ghost drops dim/faint runs, keeps normal and bold text"
}
...
test_strip_ghost_handles_combined_and_boundary_codes() {
  ...
  pass "sc_tmux_strip_ghost handles combined SGR, ESC[22m, and reset-then-dim"
}
...
test_strip_ghost_keeps_colored_text_with_2_payloads() {
  ...
  pass "sc_tmux_strip_ghost keeps colored text with 2 payloads"
}

# --- sc_pane_input_pending: dim ghost is not pending ------------------------
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" SC_FAKE_STYLED="$capture" SC_FAKE_CY=0 \
     sc_pane_input_pending "fakepane"; then
    fail "dim ghost-only composer falsely read as pending"
  fi
  pass "sc_pane_input_pending: a dim ghost-only composer is NOT pending"
}
...
test_dim_ghost_inside_bordered_composer_is_not_pending() {
  ...
  pass "sc_pane_input_pending: dim ghost inside a bordered composer is NOT pending"
}
...
test_normal_text_still_pending() {
  ...
  PATH="$fb:$PATH" SC_FAKE_STYLED="$capture" SC_FAKE_CY=0 \
    sc_pane_input_pending "fakepane" \
    || fail "real typed text was not detected as pending"
  pass "sc_pane_input_pending: normal-intensity typed text is still pending"
}
...
test_colored_text_with_2_payload_still_pending() {
  ...
  pass "sc_pane_input_pending: colored text with 2 payloads is still pending"
}
...
test_real_text_with_trailing_ghost_is_pending() {
  ...
  pass "sc_pane_input_pending: real text plus a trailing ghost run is still pending"
}

# --- sc-peek.sh stays escape-free (LLM-facing path) -------------------------
...
test_peek_output_is_escape_free() {
  ...
  # surface NONE of these escape codes into souschef's context.
  printf 'normal output line\n\xe2\x9d\xaf \033[2mpredicted next prompt\033[0m\n' > "$capture"
  # Empty SC_HOME so sc-guard.sh finds no in-flight task and stays silent.
  home="$dir/home"; mkdir -p "$home/state"
  # Pass an explicit session:window so resolution needs no metadata.
  out=$(PATH="$fb:$PATH" SC_HOME="$home" SC_FAKE_STYLED="$capture" \
        "$PEEK" "sess:win" 2>/dev/null)
  case "$out" in
    *"$ESC"*) fail "sc-peek surfaced ANSI escape codes into LLM-facing output" ;;
  esac
  # And it should still carry the real content.
  case "$out" in
    *"predicted next prompt"*) : ;;
    *) fail "sc-peek dropped pane content (expected the ghost text body as plain text)" ;;
  esac
  pass "sc-peek output is escape-free (no raw -e bytes reach souschef context)"
}
