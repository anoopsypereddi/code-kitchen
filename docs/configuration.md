...
The files and environment variables you set to operate Sous.
...
When compatible `tasks-axi` is on `PATH`, Sous uses its verbs for routine backlog mutations and keeps station chef transfers behind `fm-backlog-handoff.sh` validation; without it, backlog bookkeeping remains manual.
...
Each line records the station chef id, charter summary, absolute home path, natural-language scope, project clone list, and added date; `fm-home-seed.sh validate` refuses duplicate ids, duplicate homes, and nested or overlapping homes.
The main Sous routes by reading those scopes with judgment; the project list is provisioning data, not exclusive ownership.
Use `fm-home-seed.sh <id> - <project>...` to lease a fresh Sous worktree for the station chef home.
...
Station chef routes cover `no-mistakes` and `direct-PR` projects; `local-only` projects remain main-Sous work.
...
After creating a station chef, move existing main-backlog items that you have judged in-scope with `fm-backlog-handoff.sh <secondmate-id> <item-key>...`; it is idempotent and refuses in-flight items or non-station-chef homes.
Set `FM_SECONDMATE_CHARTER` to seed from inline charter text when no filled charter brief exists; set `FM_SECONDMATE_SCOPE` when the routing scope should differ from the charter text.

## FM_HOME

`FM_HOME` selects the operational home for one Sous instance.
...
`FM_ROOT_OVERRIDE` overrides the Sous repo root used by scripts, including the primary checkout watched by the worktree-tangle guard.
...
If compatible `tasks-axi` is already on `PATH`, bootstrap records it as an optional capability fact and Sous uses its verbs for routine backlog mutations; when it is absent or incompatible, Sous keeps hand-editing `data/backlog.md` exactly as before.
...
FM_ROOT_OVERRIDE=        # override Sous repo root and tangle-guard target; also legacy whole-root override when FM_HOME is unset
FM_STATE_OVERRIDE=       # alternate state dir, mainly for tests
FM_DATA_OVERRIDE=        # alternate data dir, mainly for tests
FM_PROJECTS_OVERRIDE=    # alternate projects dir, mainly for tests
FM_CONFIG_OVERRIDE=      # alternate config dir, mainly for tests
FM_POLL=15              # seconds between pass cycles
FM_HEARTBEAT=600        # base seconds between brigade reviews; backs off exponentially while idle
FM_HEARTBEAT_MAX=7200   # heartbeat backoff cap
FM_CHECK_INTERVAL=300   # seconds between slow checks (merged-PR polls)
FM_CHECK_TIMEOUT=30     # seconds allowed per slow check script
FM_LOCK_STALE_AFTER=2   # seconds before dead-pid lock records can be reclaimed; mid-acquire locks keep at least 2s grace
FM_GUARD_GRACE=300      # seconds before guard warnings and arm health checks treat a pass beacon as stale
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a fresh pass before reporting FAILED
FM_WATCHER_STALE_GRACE=300   # defaults to FM_GUARD_GRACE; seconds a live pass lock may have a stale beacon before re-arm errors
FM_SIGNAL_GRACE=30      # seconds to coalesce nearby status and turn-end signals into one wake
FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT=20   # seconds allowed for bootstrap's best-effort clone refresh
FM_FLEET_PRUNE=1        # set to 0 to skip pruning local branches whose upstream is gone
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.'   # busy-pane signatures, shared by pass and tmux helper
...
# sub-expediter (bin/fm-supervise-daemon.sh); presence-gated via /afk
FM_SUPERVISOR_TARGET=firstmate:0   # expediter tmux target (override; auto-discovers from $TMUX_PANE)
...
FM_INJECT_FAIL_SLEEP=30            # seconds to back off when the expediter pane is unavailable
FM_INJECT_CONFIRM_RETRIES=3        # daemon Enter-retry attempts after typing a digest once
FM_INJECT_CONFIRM_SLEEP=0.5        # seconds between daemon submit checks
FM_HEARTBEAT_SCAN_SECS=300         # cadence of the catch-all status scan for missed Chef verbs
FM_HOUSEKEEPING_TICK=15            # seconds between batch-flush, stale-recheck, and scan passes
FM_CRASH_THRESHOLD=10              # pass crashes allowed inside FM_CRASH_WINDOW before daemon backoff
...
FM_CRASH_NORMAL_SLEEP=5            # seconds to wait after an isolated pass crash
