...
The Chef drives these; interactive entrypoints work by hand too, while `*-lib.sh` files are sourced helpers.
...
| `sc-arm-command-policy.mjs` | Sole owner of Chef shell command classification (Lexer/splitProgram/commandPosition); rejects the `&`/nohup/pipeline/bundled arm mistake and direct `sc-watch.sh`/broad kill |
...
| `sc-ff-lib.sh`           | Shared guarded fast-forward helper for `/update-chef` origin pulls and no-fetch local station chef syncs       |
...
| `sc-send.sh`             | Send one verified literal line (or `--key Escape`) to a direct-report window; exits non-zero on confirmed swallowed Enter; bare `kind=secondmate` targets are marked as from-Chef; text sends pause `SC_SEND_SETTLE` seconds after success |
