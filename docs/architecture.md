...
The pass is forked into its own session (not the arm's process group), so when the Chef runs as a harness background job a process-group reap of the finished arm task cannot take the pass down with it; supervision outlives an arm teardown, and the next re-arm sees the still-live pass as healthy.
