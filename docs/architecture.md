...
A zero-token bash pass (`bin/fm-watch.sh`) sleeps on the brigade and wakes the sous-chef only when a cook reports, stalls, a PR merges, or an internal heartbeat review is due.
...
After each drain, `fm-wake-drain.sh` runs the same liveness guard as the expediting scripts, so a lapsed pass chain surfaces even on a turn that only drains and handles queued wakes.
...
Routine re-arms go through `bin/fm-watch-arm.sh`, which forks the pass as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, and prints exactly one honest status line (`started` / `healthy` / `FAILED`, the last exiting non-zero) - never a false `already running` off a dying process.
...
A pull-based guard (`bin/fm-guard.sh`) warns through expediting tool output if the primary checkout is tangled, or if tickets are in flight and that pass stops running or queued wakes are waiting to be drained.
...
A presence-gated sub-expediter (`bin/fm-supervise-daemon.sh`) extends this for walk-away expediting: the `/afk` skill activates it, after which it self-handles routine wakes in bash and escalates only Chef-relevant events as one batched, single-line digest (prefixed with an in-band sentinel marker so Sous can tell daemon injections apart from real messages).
...
`fm-send.sh` adds its own `FM_SEND_SETTLE` pause after successful text sends so immediate peeks catch the receiving turn starting; the sub-expediter uses only the shared submit core and does not pay that pause.
...
For service and prep work, `fm-spawn.sh` waits for `treehouse get` and then refuses to launch unless the pane resolves to a real git worktree root that is distinct from the project primary checkout.

The Sous repo has one extra exposure because it can dispatch cooks to work on itself.
Its operating checkout (`FM_ROOT`) and the disposable cook worktrees are all linked git worktrees of the same repository, so the valid discriminator is branch state, not whether the checkout is linked.
...
`fm-guard.sh` prints the repair command on the next brigade action, while `fm-bootstrap.sh` reports the same condition as a `TANGLE:` line at session start.
...
If returning the lease fails during 86, Sous leaves the route and home intact instead of hiding a still-held lease.
...
`local-only` projects stay with the main Sous because they merge into the main local checkout instead of a remote-backed PR path.
...
Bare `fm-send.sh fm-<id>` requests to a live `kind=secondmate` are prefixed with the from-Sous marker from `bin/fm-marker-lib.sh`, so the station chef returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
...
After seeding a station chef, `fm-backlog-handoff.sh` moves already-judged in-scope queued items from the main backlog into that station chef home so the domain queue starts in the right place.
...
Station chef homes stay on the same Sous version as the primary checkout.
On main Sous bootstrap, `fm-bootstrap.sh` fast-forwards each live station chef home recorded in `state/*.meta` to the primary default-branch commit with no origin fetch.
...
`fm-spawn.sh --secondmate` performs the same guarded local fast-forward before launch or recovery respawn; skipped syncs warn and the station chef launches unchanged.
...
`no-mistakes` projects run the full validation pipeline, `direct-PR` projects open PRs without that pipeline, and `local-only` projects stay local until Sous performs an approved fast-forward merge.
...
The full ownership rule - what is project-intrinsic versus brigade-private, and how Sous keeps the two apart without writing into project clones - is owned by Sous's operating manual in [`AGENTS.md`](../AGENTS.md) (project memory ownership).
...
`/updatefirstmate` fast-forwards the running Sous repo and registered station chef homes from `origin`, then re-reads updated instructions and nudges updated station chefs without touching project clones.
...
The mechanics are owned by the `/updatefirstmate` skill and Sous's operating manual in [`AGENTS.md`](../AGENTS.md) (self-update).

## Restart-proof

All state lives in tmux, status files, local markdown under `data/`, `data/secondmates.md`, and persistent station chef homes.
Kill the Sous session anytime; the next one reconciles and carries on.
...
The presence-gated sub-expediter (`bin/fm-supervise-daemon.sh`) provides proactive wake routing for walk-away expediting via the `/afk` skill; a blocking-waiter split remains a deferred follow-up phase.
