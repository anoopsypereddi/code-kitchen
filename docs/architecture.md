...
How Chef works, in depth.

The [README](../README.md) carries the high-level overview and a short synopsis.
This document expands every part of it.
Chef's full operating manual for the orchestrator agent itself is [`AGENTS.md`](../AGENTS.md); this is the human-facing companion.

## Event-driven expediting

A zero-token bash pass (`bin/sc-watch.sh`) sleeps on the brigade and wakes the Chef only when a cook reports, stalls, a PR merges, or an internal heartbeat review is due.
...
The pass is a normal same-process-group child of the arm (never setsid-detached into its own session), and the arm blocks in `wait` on it, so the harness-tracked task IS the arm and the pass's lifetime is tied to it: a wake exits the pass, the arm relays it and exits, and the harness's notify-on-exit drives the next re-arm; a harness reap of the arm (a pid signal or a process-group reap) takes the pass down with it via the arm's signal trap, so no beating-but-untracked pass is ever orphaned to fool the next re-arm into `healthy` and then die silently. (An earlier design detached the pass to survive a reap of a "finished" arm, but the arm never finishes independently - it `wait`s - so that detach only orphaned the pass on every reap and broke the notify-on-exit contract.)
...
A presence-gated sub-expediter (`bin/sc-supervise-daemon.sh`) extends this for walk-away expediting: the `/afk` skill activates it, after which it self-handles routine wakes in bash and escalates only Chef-relevant events as one batched, single-line digest (prefixed with an in-band sentinel marker so Chef can tell daemon injections apart from real messages).
...
The Chef repo has one extra exposure because it can dispatch cooks to work on itself.
...
If returning the lease fails during 86, Chef leaves the route and home intact instead of hiding a still-held lease.
...
`local-only` projects stay with the main Chef because they merge into the main local checkout instead of a remote-backed PR path.
...
Bare `sc-send.sh sc-<id>` requests to a live `kind=secondmate` are prefixed with the from-Chef marker from `bin/sc-marker-lib.sh`, so the station chef returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
...
Station chef homes stay on the same Chef version as the primary checkout.
On main Chef bootstrap, `sc-bootstrap.sh` fast-forwards each live station chef home recorded in `state/*.meta` to the primary default-branch commit with no origin fetch.
...
`direct-PR` projects (the default) have the cook validate locally - running the project's lint/format/type/test green - then open a PR with `gh` for the Chef to merge; `local-only` projects stay local until Chef performs an approved fast-forward merge.
...
The full ownership rule - what is project-intrinsic versus brigade-private, and how Chef keeps the two apart without writing into project clones - is owned by Chef's operating manual in [`AGENTS.md`](../AGENTS.md) (project memory ownership).
...
`/update-chef` fast-forwards the running Chef repo and registered station chef homes from `origin`, then re-reads updated instructions and nudges updated station chefs without touching project clones.
...
The mechanics are owned by the `/update-chef` skill and Chef's operating manual in [`AGENTS.md`](../AGENTS.md) (self-update).

## Restart-proof

All state lives in tmux, status files, local markdown under `data/`, `data/secondmates.md`, and persistent station chef homes.
Kill the Chef session anytime; the next one reconciles and carries on.
